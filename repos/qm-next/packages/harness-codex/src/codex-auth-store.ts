...
import type { CredentialFile, Keychain } from '@qm/types'
import { swallow } from '@qm/harness-pi'
import {
...
export function codexOAuthAccessTokenExpiresAt(
...
async function refreshCodexOAuth(
  ...
  return codexOAuthAuthFromValue(next) &&
    codexOAuthJwtAccountId(next) === codexOAuthJwtAccountId(auth)
    ? next
    : null
}
...
function codexAuthFromFiles(
  files: CredentialFile[],
): { path: string; auth: JsonObject } | null {
  for (const file of files) {
    const normalized = file.path.replace(/^\.\//, '')
    if (
      !CODEX_AUTH_FILE_PATHS.includes(normalized) &&
      !normalized.endsWith('/auth.json')
    )
      continue
    try {
      const auth = codexOAuthAuthFromValue(
 ...
 */
export function keychainCodexAuthStore(
  deps: KeychainCodexAuthStoreDeps,
): CodexAuthStore {
  ...
  const readCurrent = async (): Promise<{
    ...
    return found
      ? { ownerId: meta.ownerId, service: meta.service, ...found }
      : null
  }
  ...
  const persist = async (
    ...
    if (!latest || codexOAuthRefreshToken(latest.auth) !== replacedRefreshToken)
      return false
    const nextExpiresAt = codexOAuthAccessTokenExpiresAt(next)
    await deps.keychain.save({
      ...
      files: [
        {
          path: current.path,
          contentBase64: Buffer.from(JSON.stringify(next), 'utf8').toString(
            'base64',
          ),
        },
      ],
    ...
    async load(): Promise<JsonObject | null> {
      ...
      refreshing ??= (async () => {
        try {
          ...
          if (!next) return null
          const lock = await acquireCodexOAuthAuthLock(
            path,
            undefined,
            10_000,
            25,
          ...
          try {
            ...
            if (
              !latest ||
              codexOAuthRefreshToken(latest) !== codexOAuthRefreshToken(current)
            )
              return latest
