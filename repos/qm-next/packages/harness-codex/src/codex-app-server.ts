...
import { errMessage } from '@qm/harness-pi'
...
function diagnosticKeyIsSensitive(key: string): boolean {
  return CODEX_DIAGNOSTIC_SENSITIVE_KEYS.has(
    key.toLowerCase().replace(/[^a-z]/g, ''),
  )
}

function redactStructuredDiagnosticsValue(
  value: unknown,
  sensitive = false,
): unknown {
  if (sensitive) return '[redacted]'
  if (Array.isArray(value))
    return value.map((item) => redactStructuredDiagnosticsValue(item))
...
export class CodexAppServer {
  ...
  private readonly pending = new Map<
    JsonRpcId,
    {
      ...
      validate?: JsonRpcResultValidator<unknown>
    }
  >()
  ...
  request<T>(
    method: string,
    params: unknown,
    validate: JsonRpcResultValidator<T>,
    signal?: AbortSignal,
  ): Promise<T>
  request<T>(
    method: string,
    ...
    const validate =
      typeof validateOrSignal === 'function' ? validateOrSignal : undefined
    let requestSignal: AbortSignal | undefined
    if (validate) requestSignal = signal
    else if (typeof validateOrSignal !== 'function')
      requestSignal = validateOrSignal
    if (requestSignal?.aborted)
      return Promise.reject(new Error('Codex app-server request cancelled'))
    ...
    const result = new Promise<unknown>((resolve, reject) => {
      ...
      this.pending.set(id, {
        ...
        reject,
        ...(validate
          ? { validate: validate as JsonRpcResultValidator<unknown> }
          : {}),
      })
    })
    if (requestSignal) {
      const onAbort = () => {
        ...
        if (this.cancelledRequestIds.size >= MAX_CANCELLED_REQUEST_IDS) {
          this.failAll(
            new CodexRpcError(
              'Codex app-server exceeded its cancelled request limit',
            ),
          )
          this.process.kill('SIGTERM')
        } else {
  ...
  private async receive(line: string): Promise<void> {
    ...
    if (message.id !== undefined && !message.method) {
      ...
      if (!waiter) {
        ...
        throw new CodexRpcError(
          `Codex app-server sent an unknown response id ${String(message.id)}`,
        )
      }
      ...
      if ('error' in message) {
        if (!message.error || typeof message.error !== 'object') {
          waiter.reject(
            new CodexRpcError('Codex app-server response has an invalid error'),
          )
          return
        }
        waiter.reject(
        ...
        if (waiter.validate && !waiter.validate(message.result)) {
          waiter.reject(
            new CodexRpcError(
              'Codex app-server response has an invalid result',
            ),
          )
          return
        }
        ...
        waiter.reject(
          new CodexRpcError(
            'Codex app-server response is missing result or error',
          ),
        )
      return
    }
    ...
    try {
      ...
      await this.send({
        id: message.id,
        error: { code: -32000, message: errMessage(error) },
      })
    }
  }
