...
export function codexOAuthJwtAccountIdFromToken(
  value: unknown,
): string | undefined {
  ...
  try {
    ...
    const claims = payload
      ? asObject(payload['https://api.openai.com/auth'])
      : null
    return typeof claims?.chatgpt_account_id === 'string' &&
...
export function codexAuthFileForEnv(
...
function isCodexOAuthAuth(value: unknown): value is JsonObject {
  ...
  if (
    ...
    !CODEX_OAUTH_MODES.has(auth.auth_mode)
  )
    return false
    ...
    if (key === 'tokens') {
      ...
      if (tokens) {
        copy.tokens = Object.fromEntries(
          ['access_token', 'refresh_token', 'id_token', 'account_id'].flatMap(
            (token) =>
...
export function codexOAuthRefreshToken(value: unknown): string | undefined {
  const auth = asObject(value)
  const tokens = auth ? asObject(auth.tokens) : null
  return typeof tokens?.refresh_token === 'string' && tokens.refresh_token
    ? tokens.refresh_token
    : undefined
}
