...
import type { Harness, HarnessTurnInput, HarnessTurnResult } from '@qm/types'
import { DEFAULT_CODEX_MODEL_ID, modelSupportedByHarness } from '@qm/model'
import {
  CONFIG_DEFAULTS,
...
export type CodexTaskStatus = 'pending' | 'in_progress' | 'completed' | 'failed'

export interface CodexTaskStore {
  create(task: {
    id: string
    sessionId: string
    originRunId: string
  ...
  transitionStatus(
    id: string,
    expected: CodexTaskStatus,
    next: CodexTaskStatus,
    runId: string,
  ): Promise<unknown>
}
...
const CODEX_TERMINAL_TURN_STATUSES = new Set([
  'completed',
  'failed',
  'interrupted',
  'cancelled',
...
function isCodexThreadStart(
  value: unknown,
): value is { thread: { id: string }; model?: string } {
...
type ActiveTurn = {
  ...
  taskStatuses: Map<string, CodexTaskStatus>
...
export function codexProviderFailure(message: string): Error {
  ...
  return codexNonRetryable(safe)
    ? new NonRetryableTurnError(safe)
    : new Error(safe)
}
...
function sumUsage(
  byThread: ReadonlyMap<string, LlmCallUsage>,
): LlmCallUsage | null {
  if (!byThread.size) return null
  const total: LlmCallUsage = {
    input: 0,
    output: 0,
    cacheRead: 0,
    cacheWrite: 0,
...
export function codexTokenUsageUpdate(
  ...
  const totalInputTokens = usageNumber(
    usage.total,
    'inputTokens',
    'input_tokens',
  )
  ...
  return {
    inputTokens: lastInputTokens || totalInputTokens - priorInputTokens,
    totalInputTokens,
  }
}
...
export function prepareCodexHome(
  ...
  if (oauthAuth) {
    ...
    writeFileSync(
      ...
      JSON.stringify(childCodexOAuthAuth(oauthAuth)),
      { mode: 0o600 },
    )
    return target
  }
...
async function transitionTask(
  store: CodexTaskStore | undefined,
  id: string,
  expected: CodexTaskStatus,
  next: CodexTaskStatus,
  runId: string,
  ...
  if (!updated)
    throw new Error(
      `task ${id} was not ${expected} while transitioning to ${next}`,
    )
}

function toolOptions(
  opts: CodexHarnessOptions,
  turn?: HarnessTurnInput,
): PiToolsOptions {
  return {
    ...(opts.scratchExec !== undefined
      ? { scratchExec: opts.scratchExec }
      : {}),
    ...(opts.ownerAuthExec !== undefined
      ? { ownerAuthExec: opts.ownerAuthExec }
      : {}),
    ...(opts.reachExec !== undefined ? { reachExec: opts.reachExec } : {}),
    ...(opts.mcpTools ? { mcpTools: opts.mcpTools } : {}),
    ...(opts.controlTools !== undefined
      ? { controlTools: opts.controlTools }
      : {}),
    ...(opts.execTimeoutMs !== undefined
      ? { execTimeoutMs: opts.execTimeoutMs }
      : {}),
    ...(opts.execTimeoutCeilingMs !== undefined
      ? { execTimeoutCeilingMs: opts.execTimeoutCeilingMs }
      : {}),
    ...(opts.backgroundJobTtlMs !== undefined
    ...
    ...(turn
      ? {
          ...(turn.readOnly !== undefined ? { readOnly: turn.readOnly } : {}),
          ...(turn.surfaceTools !== undefined
            ? { surfaceTools: turn.surfaceTools }
            : {}),
          ...(turn.surfaceName !== undefined
      ...
      : { surfaceTools: true, surfaceName: 'api' }),
  }
}
...
function textFromTurn(turn: CodexTurn): string {
  ...
  const unphased = messages.filter(
...
export function codexTaskTitle(prompt: unknown): string {
  ...
  const named = /\bYou are (?:the )?([^.!?]{1,80}? subagent)\b/i.exec(
    normalized,
  )?.[1]
...
export function codexTurnInputText(
  turn: Pick<
    HarnessTurnInput,
    'history' | 'priorTurns' | 'input' | 'environment'
  >,
): string {
  const prior = turn.history.length
    ...
    : seedPriorTurns(turn.priorTurns ?? [])
        ...
        .join('\n')
  return [prior, turn.input, turn.environment]
...
export function createCodexHarness(opts: CodexHarnessOptions = {}): Harness {
  ...
  const resolveModelId = (scope?: ScopeId) =>
    [
      typeof configuredModel === 'function'
        ? configuredModel(scope)
        : configuredModel,
      opts.defaultModelId,
      DEFAULT_CODEX_MODEL_ID,
    ].find((id): id is string => modelSupportedByHarness(id, 'codex'))!
  const defaultTurnWallClockMs =
    opts.turnWallClockMs ?? CONFIG_DEFAULTS.turnWallClockSec * 1000
  const sourceEnv = opts.env ?? {}
  const authPath = codexAuthFileForEnv(sourceEnv, true)
  const authStore: CodexAuthStore | undefined =
    opts.authStore ??
    (authPath && readCodexOAuthAuthFile(authPath)
      ? fileCodexAuthStore(authPath)
      : undefined)
  ...
  const processCollabItem = async (
    ...
    for (const [receiver, agent] of Object.entries(agents)) {
      ...
      if (!taskId) continue
      let next: CodexTaskStatus | undefined
      if (agent.status === 'completed' || agent.status === 'shutdown')
        next = 'completed'
      else if (
  ...
  const buildServer = (
    ...
    const server: CodexAppServer = new CodexAppServer({
      ...
      onNotification: async (method, params) => {
        ...
        if (method === 'thread/tokenUsage/updated') {
          ...
          const usage = codexTokenUsageUpdate(
            p,
            state.usageInputTotals.get(threadId),
          )
          if (!usage) return
        ...
        if (method === 'turn/completed' && threadId === state.threadId) {
          ...
          if (!isCodexTurn(completed)) {
            state.reject(
              new CodexRpcError(
                'Codex app-server sent an invalid turn/completed payload',
              ),
            )
            return
          }
          state.resolve(
            completed.items?.length
              ? completed
              : { ...completed, items: state.completedItems },
          )
        }
      },
      onRequest: async (method, params) => {
        ...
        const state = active.get(threadId)
        if (!state || state.server !== server)
          throw new Error('inactive Codex thread')
  ...
  const ensureRuntime = async (
    ...
    if (runtime) {
      ...
      const staleError =
        stale.server.error() ??
        new Error('Codex app-server exited during a turn')
      ...
      const promise = (async () => {
        ...
        try {
          if (oauthConfigured && !sourceAuth)
            throw new Error(
              `Codex OAuth auth is unavailable (${authStore!.description})`,
            )
          prepareCodexHome(
            sourceEnv,
            jail,
            oauthConfigured ? sourceAuth : undefined,
          )
          if (startupAbort.signal.aborted)
            throw new Error('Codex app-server startup cancelled')
          server = buildServer(
            jail,
            codexChildEnv(
              sourceEnv,
          ...
          await Promise.race([
            ...
            new Promise<never>((_, reject) => {
              startTimer = setTimeout(
                () =>
                  reject(
                    new Error('Codex app-server initialization timed out'),
                  ),
  ...
  const runPrompt = async (
    ...
    const runtimeRecoveryDeadline =
      Date.now() + Math.max(wallMs, CODEX_START_TIMEOUT_MS)
    const setupCancelled = new Error('Codex setup cancelled')
    const setupTimedOut = new NonRetryableTurnError(
      `Codex turn exceeded ${Math.round(wallMs / 1000)}s wall clock`,
    )
    ...
    const setupTimer =
      wallMs > 0
        ? setTimeout(() => stopSetup(setupTimedOut), wallMs)
        : undefined
    const finishSetup = () => {
      setupSettled = true
    ...
    const awaitSetup = <T>(operation: Promise<T>): Promise<T> =>
      Promise.race([operation, setupStop])
      ...
      if (!ephemeral && oauthConfigured) {
        ...
        if (runtime !== rt || rt.server.process.exitCode !== null) {
          if (Date.now() >= runtimeRecoveryDeadline)
            throw new NonRetryableTurnError(
              'Codex OAuth runtime recovery timed out',
            )
          rt = await awaitSetup(
            ensureRuntime((release) => {
              releaseStartupWaiter = release
    ...
    try {
      ...
      model = modelSupportedByHarness(turn.model, 'codex')
        ? turn.model!
        : resolveModelId(turn.scopeLabel)
      const reasoningEffort = codexReasoningEffort(turn.thinkingLevel)
      threadStartRequest = {
        ...(model ? { model } : {}),
        cwd: rt.jail,
        ...
        Promise.race([
          ...
          new Promise<never>((_, reject) => {
            requestTimer = setTimeout(() => {
              requestAbort.abort()
              reject(
                new NonRetryableTurnError(
                  'Codex thread/start request timed out',
                ),
              )
            }, requestTimeoutMs)
          }),
    ...
    try {
      ...
      if (replay.length)
        await awaitSetup(
          rt.server.request('thread/inject_items', { threadId, items: replay }),
        )
      userEntry = await awaitSetup(
        turn.emit({
          ...
          payload: {
            text: turn.input,
            ...((turn.triggerTs ?? turn.entryTs)
              ? { ts: turn.triggerTs ?? turn.entryTs }
              : {}),
    ...
    try {
      ...
      const turnStartTimeoutMs = deadline
        ? Math.max(1, deadline - Date.now())
        : CODEX_START_TIMEOUT_MS
      ...
      const response = await Promise.race([
        ...
        new Promise<never>((_, reject) => {
          turnStartTimer = setTimeout(() => {
            ...
            reject(
              new NonRetryableTurnError('Codex turn/start request timed out'),
            )
          }, turnStartTimeoutMs)
        }),
      ])
        ...
        .catch((error: unknown) => {
          if (turnStartTimedOut) {
            const timeoutError = new NonRetryableTurnError(
              'Codex turn/start request timed out',
            )
            timeoutError.cause = error
            throw timeoutError
          }
          throw error instanceof CodexRpcError
            ? codexProviderFailure(error.message)
            : error
        })
      turnId = response.turn.id
      if (toolAbort.signal.aborted || turn.cancel?.aborted)
  ...
  const single = async (
    ...
    const session = {
      id: `oneshot-${randomBytes(8).toString('hex')}`,
  ...
  return defineHarness(
    ...
    },
    {
      ...
      close: async () => {
        ...
        if (current || active.size) {
          for (const state of active.values())
            state.reject(new Error('Codex harness closed during a turn'))
          active.clear()
        }
        if (current) {
      ...
      judge: (system, prompt) =>
        single(system, prompt, undefined, undefined, judgeModelId),
      ...
      generateTitle: async (transcript) =>
        sanitizeTitle(
          await single(TITLE_GENERATION_PROMPT, titleUserPrompt(transcript)),
        ),
