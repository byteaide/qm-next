/**
 * Renderer projection of the session tape (M-Tape-1).
 *
 * qm-verbatim port of qm `src/harness/tape-projection.ts` (539L). The port
 * is a render-only view — it reads tape rows (and falls back to entry
 * reconstruction when coverage is missing), settles them into the same
 * `SessionEntry[]` shape `foldTape(tape, audience)` would produce for the
 * model, and emits admin/search/visibility projections on top.
 *
 * Source-of-truth (qm, 2026-09-26): `repos/qm/src/harness/tape-projection.ts`.
 *
 * Key contracts consumed (qm-verbatim, declared in `@qm/types`):
 * - `TAPE_RENDER_VERSION` / `RENDER_IMPORT_EVENT` — `packages/types/src/tape.ts`
 * - `createContextSummaryPayload` — used for `compaction` projection
 * - `entryWithinTenure` + `ParticipantWindow` — audience filtering in
 *   `createTranscriptSource.forViewer`
 * - `SEARCHABLE_ENTRY_TYPES` + `entrySearchText` + `entrySearchAuthor` +
 *   `NewSearchEntry` — `searchRowsFromEntries` admin/search rows
 * - `deliveryNoteManifest` + `legacyDeliveryNoteManifest` — delivery-entry
 *   projection in `userDraft`
 *
 * Independence from `@qm/harness-pi`: per M-Tape-1 placement decision
 * (2026-09-26), the projection lives in `packages/store/` and does not
 * import harness-pi. The three private helpers (`assistantDroppedAtReplay`,
 * `textFromContent`, `thinkingBlocksFromContent`) are inlined from qm-next
 * `harness-pi/src/{tape-fold.ts, pi-harness.ts}` — they are pure functions
 * over message shape and identical to the harness-pi copies, so a future
 * refactor can move them to `@qm/types` if other store code needs them.
 *
 * Byte-identity gate (M-Tape-3): `pnpm check:tape-renderer` asserts
 * `foldTape(tape) === forRender(tape).entries` over seeded fixtures.
 */
import type {
  EntryType,
  GetEntriesOptions,
...
// ---------------------------------------------------------------------------
// Store-side projection helpers (qm-verbatim port of three private helpers
// in qm-next `harness-pi/src/{tape-fold.ts, pi-harness.ts}`; inlined to keep
// `@qm/store` independent of `@qm/harness-pi`).
// ---------------------------------------------------------------------------
...
function thinkingBlocksFromContent(content: unknown): ThinkingBlock[] {
  if (!Array.isArray(content)) return []
...
function swallow(_context: string, _err: unknown): void {
  // Minimal local swallow — projection read failures are non-fatal and
  // must not propagate; callers fall back to entry reconstruction.
  // Mirrors the shape of `@qm/harness-pi/src/errors.ts` `swallow` but
  // intentionally side-effect-free here (the projection path is hot and
  // a console.warn would balloon logs under sustained fan-out).
}

// ---------------------------------------------------------------------------
// Row-level types (qm `tape-projection.ts:14-41`)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Renderable slice helpers (qm `tape-projection.ts:66-116`)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Projection main loop (qm `tape-projection.ts:193-395`)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// `createTranscriptSource` (qm `tape-projection.ts:397-521`)
// ---------------------------------------------------------------------------
...
export function createTranscriptSource(sessions: TranscriptStore): TranscriptSource {
  const projected = async (
    ...
    try {
      ...
      if (sessions.getTranscriptEntries) {
        ...
        const getOpts: GetEntriesOptions = {
          ...(limit !== undefined ? { limit } : {}),
          ...(beforeSeq !== undefined ? { beforeSeq } : {}),
          ...(sinceSeq !== undefined ? { sinceSeq } : {}),
        }
        const [entries, completePrefix] = await Promise.all([
          sessions.getTranscriptEntries(sessionId, getOpts),
          sinceSeq === undefined || first === 0 || (await sessions.canReadTranscriptSuffix?.(sessionId, first)) === true,
        ])
  ...
  return {
    async forRender(sessionId, opts?): Promise<TranscriptRead> {
      ...
      if (read === null) {
        const fallbackOpts: GetEntriesOptions = {
          ...(opts?.beforeSeq !== undefined ? { beforeSeq: opts.beforeSeq } : {}),
          ...(opts?.limit !== undefined ? { limit: opts.limit } : {}),
        }
        const rows = await sessions.getEntries(sessionId, fallbackOpts)
...
// ---------------------------------------------------------------------------
// `searchRowsFromEntries` (qm `tape-projection.ts:523-538`)
// ---------------------------------------------------------------------------
