/**
 * Replay dedupe (qm `src/auth/replay-dedupe.ts`): single-claim semantics
 * for source-auth event ids. The Postgres store is durable — a restart
 * cannot resurrect a spent sign-in link; the memory store exists for tests
 * and reports `durable: false` so gates can answer qm's not-durable shape.
 */
