...
export function mintCapabilityToken(claims: CapabilityClaims, secret: string, orgId: string): Promise<string> {
...
export async function verifyCapabilityToken(token: string, secret: string | string[], now: number = Date.now()): Promise<CapabilityClaims | null> {
  ...
  if (!claims || typeof claims.actorId !== 'string' || typeof claims.scopeId !== 'string' || typeof claims.exp !== 'number') {
    return null
  }
