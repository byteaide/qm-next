/**
 * Signed payload tokens: HS256 JOSE compact signatures with key-id based
 * secret rotation, plus verification of the legacy `payload.hmac` format so
 * tokens minted by qm-era tooling keep verifying during migration. Ported
 * from `qm/src/auth/signed-token.ts`.
 */
