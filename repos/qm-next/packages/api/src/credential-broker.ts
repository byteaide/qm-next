/**
 * Credential broker (qm `src/api/credential-broker.ts`): proxies an
 * entitled, audience-gated agent call against a pinned service credential —
 * host pinning, method/path allowlists, header injection and usage/audit
 * records included.
 */
import type { CapabilityClaims } from '@qm/auth'
import type { CredentialUsageSink } from '@qm/admin'

interface DecryptedServiceCredential {
  slug: string
  name: string
  secret: string
  delivery: string
...
export type ServiceCredentialReader = {
  getServiceCredentialSecret(orgScopeId: string, slug: string): Promise<DecryptedServiceCredential | null>
}
