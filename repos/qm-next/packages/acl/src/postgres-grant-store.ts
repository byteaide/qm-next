/**
 * Postgres grant store (qm `src/acl/postgres-grant-store.ts`):
 * `GrantPersistence` backed by `acl_grants` with an `acl_grants_version`
 * bump-table that lets the in-memory cache invalidate on any write to
 * the same Postgres instance without taking a session lock on reads.
 *
 * `replaceForResourceIfCurrent` runs under `pg_advisory_xact_lock`
 * keyed on the resource, so two concurrent replaces see each other's
 * state rather than racing on `SELECT … FOR UPDATE` + `DELETE`.
 */
import { createPgPool } from '@qm/store'
import type { Permission, ScopeId } from '@qm/types'
import { createMemoryGrantPersistence, type Grant, type GrantPersistence } from './acl-store.ts'
...
export interface PostgresGrantStore extends GrantPersistence {}
  ...
  return {
    ...
    async replaceForResourceIfCurrent(
      ...
      try {
        ...
        const current = selected.rows.map((row) => rowToGrant(row as Record<string, unknown>))
        const sameTuple = (a: Grant, b: Grant): boolean =>
        ...
        await client.query('DELETE FROM acl_grants WHERE owner_scope_id = $1 AND path = $2', [
          ownerScopeId,
          ref,
        ])
...
export { createMemoryGrantPersistence }
export type { Grant, GrantPersistence }