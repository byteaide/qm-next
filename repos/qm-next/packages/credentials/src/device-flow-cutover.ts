...
export interface DeviceFlowCutoverStore {
  ...
  set(scope: ScopeId, service: string, mode: DeviceFlowCutoverMode, updatedBy: string): Promise<DeviceFlowCutoverPolicy>
...
export function createDeviceFlowCutoverStore(
  ...
  opts: {
    ...
    now?: () => number
    resetId?: () => string
    resets?: DurableMap<DeviceFlowCutoverReset>
  } = { orgId: '' },
): DeviceFlowCutoverStore {
  const orgScope: ScopeId = `org:${opts.orgId}`
