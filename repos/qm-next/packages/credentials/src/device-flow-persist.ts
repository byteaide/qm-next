import type { Keychain, Sandbox, SandboxHandle, ScopeId } from '@qm/types'
import { scopeId } from '@qm/types'
import { parseTar } from './tar.ts'
import { shq } from './shell.ts'
import { fileCredentialFingerprint } from './keychain.ts'
import { credentialServiceForPath, EPHEMERAL_CRED_DIR, type CredentialPathSpec } from './resident-paths.ts'
...
export async function captureDeviceFlowLogins(input: DeviceFlowPersistInput): Promise<string[]> {
  ...
  const byService = new Map<string, Array<{ path: string; contentBase64: string }>>()
