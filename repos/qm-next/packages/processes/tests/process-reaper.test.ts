/**
 * Process reaper tests (parity 16.0): flips expired sessions, retries on
 * transient kill failures, only the leader reaps, and the TERM→KILL
 * escalation runs against a fake ProcessSandbox.
 */
...
import { createMemoryProcessRegistry, type ProcessRecord } from '../src/index.ts'
import { createProcessReaper, createReaperKillHook, type ReaperLeaderLease } from '../src/index.ts'
...
function killSpySandbox(opts?: { diesOn?: 'TERM' | 'KILL' | 'never'; vanished?: boolean | 'string-throw' }) {
  ...
  const sandbox = {
    ...
    async provision(layers: WorkspaceLayer[], provOpts?: ProvisionOptions) {
      provisions.push({ layers, ...(provOpts ? { opts: provOpts } : {}) })
      return handle
    },
    async readProcess(_h: SandboxHandle, processId: string) {
      ...
      const status: ProcessState = exited.has(processId) ? { state: 'exited', code: 143 } : { state: 'running' }
      return { chunks: '', cursor: 0, status }
    },
    ...
    async teardown(_h: SandboxHandle, tOpts?: TeardownOptions) {
      teardowns.push(tOpts ? { opts: tOpts } : {})
    },
    async startProcess() {
...
test('reaper calls onReaped for each reaped record (so a reaped run death can notify the conversation)', async () => {
  const reg = createMemoryProcessRegistry()
  ...
  const reaper = createProcessReaper(reg, {
    intervalMs: 60_000,
    onReaped: async (r: ProcessRecord) =>
      void seen.push({ kind: r.kind, ...(r.runId ? { runId: r.runId } : {}) }),
  })
...
test('a row deleted mid-sweep is neither counted nor notified', async () => {
  const reg = createMemoryProcessRegistry()
...
test('only the leader instance interval sweep reaps (one replica kills, not all of them)', async () => {
  const reg = createMemoryProcessRegistry()
  await reg.register({ processId: ID, scopeId: 's', kind: 'background', command: 'bg: x', ttlMs: -1 })

  let leader = false
  const lease: ReaperLeaderLease = {
  ...
  try {
    await sleep(40)
    assert.equal((await reg.get(ID))!.status, 'running', 'a non-leader interval does not reap')
    assert.equal(killed.length, 0)

    leader = true
    await sleep(40)
    assert.equal((await reg.get(ID))!.status, 'reaped', 'the leader interval reaps the expired process')
