/**
 * Process session registry (qm `src/processes/process-registry.ts`).
 *
 * Tracks long-running processes (build, dev-server, background) registered
 * by the orchestrator so they can outlive a turn: a single source of truth
 * for live/expired/reaped state across instances. Memory and Postgres
 * dual impl with the same surface.
 */
import { createPgPool, errMessage, type PgPool } from '@qm/store'
...
export interface PostgresProcessRegistry extends ProcessRegistry {
  close(): Promise<void>
}

export function createPostgresProcessRegistry(connectionString: string): PostgresProcessRegistry {
  const { q, close }: PgPool = createPgPool(connectionString, [
  ...
  return {
    async register(rec) {
      ...
      try {
        ...
        throw new Error(`process registry insert failed: ${errMessage(e)}`)
      }
      return row
    },
    async get(processId) {
    ...
    async listByScope(scopeId) {
      const rows = await q('SELECT * FROM process_sessions WHERE scope_id = $1', [scopeId])
    ...
    async markStatus(processId, status) {
      const updated = await q(
        'UPDATE process_sessions SET status = $1 WHERE process_id = $2 RETURNING process_id',
        [status, processId],
      )
