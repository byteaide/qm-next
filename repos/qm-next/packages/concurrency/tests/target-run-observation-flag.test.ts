...
test('slice-1.5: complete on a target row does NOT write status=done', async () => {
  ...
  assert.ok(claimed)
  const ok = await store.complete(claimed.id, claimed.leaseToken, { status: 'ok', reply: 'all good', sessionId: 'session-A' })
...
test('slice-1.5: terminal listener fires for target rows via targetState', async () => {
  ...
  const claimed = await store.claim('worker-1', 60_000)
  assert.ok(claimed)
  await store.complete(claimed.id, claimed.leaseToken, { status: 'ok', reply: 'all good', sessionId: 'session-A' })
