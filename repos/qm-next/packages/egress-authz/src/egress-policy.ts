/**
 * Egress policy (qm `src/resolution/egress-policy.ts`): allow/deny
 * list parsing with input validation, host-normalization, suffix
 * matching (`example.com` matches `api.example.com`), and the
 * decision function used by the authz server.
 */
import type { EgressPolicy } from '@qm/types'
...
function normalizeHost(host: string): string {
  ...
  try {
    return new URL(`https://${isIPv6 ? `[${s}]` : s}`).hostname
      .replace(/^\[(.*)\]$/, '$1')
      .replace(/\.$/, '')
  } catch {
    return s
