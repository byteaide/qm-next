...
import type { ScopeId, SessionEntry, ToolContext } from '@qm/types'
import type { McpToolDescriptor } from '@qm/types'
import type { Harness, HarnessTurnInput, HarnessTurnResult } from '@qm/types'
import { modelSupportedByHarness, modelSupportsFastMode, contextTokenBudgetForModel, DEFAULT_AGENT_MODEL_ID } from '@qm/model'
import {
  CONFIG_DEFAULTS,
...
export type ClaudeTaskStatus = 'pending' | 'in_progress' | 'completed' | 'failed'
...
export interface ClaudeTaskStore {
  ...
  transitionStatus(id: string, expected: ClaudeTaskStatus, next: ClaudeTaskStatus, runId: string): Promise<unknown>
}
...
async function transitionTask(
  store: ClaudeTaskStore | undefined,
  id: string,
  expected: ClaudeTaskStatus,
  next: ClaudeTaskStatus,
  runId: string,
): Promise<void> {
  if (!store) return
...
function toolOptions(opts: ClaudeHarnessOptions, turn?: HarnessTurnInput): PiToolsOptions {
  return {
    ...(opts.scratchExec !== undefined ? { scratchExec: opts.scratchExec } : {}),
    ...(opts.ownerAuthExec !== undefined ? { ownerAuthExec: opts.ownerAuthExec } : {}),
    ...(opts.reachExec !== undefined ? { reachExec: opts.reachExec } : {}),
    ...(opts.mcpTools ? { mcpTools: opts.mcpTools } : {}),
    ...(opts.controlTools !== undefined ? { controlTools: opts.controlTools } : {}),
    ...
    ...(turn
      ? {
          ...
          ...(turn.surfaceTools !== undefined ? { surfaceTools: turn.surfaceTools } : {}),
          ...(turn.surfaceName !== undefined ? { surfaceName: turn.surfaceName } : {}),
          ...(turn.credentialExecServices ? { credentialExecServices: turn.credentialExecServices } : {}),
        }
...
export function createClaudeHarness(opts: ClaudeHarnessOptions = {}): Harness {
  ...
  const runPrompt = async (turn: HarnessTurnInput, toolsEnabled = true): Promise<HarnessTurnResult> => {
    ...
    const taskStates = new Map<string, { callId: string; status: ClaudeTaskStatus; resultEmitted: boolean }>()
    ...
    const effortLevel = effort(turn.thinkingLevel)
      ...
      const consume = (async () => {
        for await (const message of sdkQuery) {
          ...
          if (message.type === 'system' && message.subtype === 'task_updated') {
            ...
            let next: ClaudeTaskStatus | undefined
          ...
          if (message.type === 'system' && message.subtype === 'task_notification') {
            const tracked = taskStates.get(message.task_id)
            if (tracked) {
              const next: ClaudeTaskStatus = message.status === 'completed' ? 'completed' : 'failed'
