/**
 * pi model resolution: the static registry, clone models, codex-subscription
 * namespacing, harness support/serviceability matrices, and default-model
 * selection. Translated from qm's pi-models onto the frozen ProviderKeys and
 * ModelProviderAvailability vocabulary; custom providers and provider
 * endpoint overrides resolve through the same choke point as qm.
 */
...
function cloneModel(model: PiModel, id: string, name: string, overrides: Partial<PiModel> = {}): PiModel {
  ...
  if (model.headers) next.headers = { ...model.headers }
  if (model.thinkingLevelMap) next.thinkingLevelMap = { ...model.thinkingLevelMap }
  if (model.compat) next.compat = { ...(model.compat as Record<string, unknown>) } as NonNullable<PiModel['compat']>
  return next
}
