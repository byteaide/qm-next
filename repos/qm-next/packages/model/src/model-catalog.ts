...
import { modelSupportedByHarness, registerOpenRouterCatalogModel, resolveModel, SELECTABLE_BASE_MODELS } from './pi-models.ts'
...
export async function selectableModelCatalog(fetcher: typeof fetch = fetch): Promise<ModelCatalogEntry[]> {
  ...
  const entry: CacheEntry = existing ?? { expiresAt: 0, models: [] }
