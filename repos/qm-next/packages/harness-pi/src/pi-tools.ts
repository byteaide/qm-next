import { defineTool, type ToolDefinition } from '@qm/pi-coding-agent'
...
import { classifyScopeLabel } from './classify-scope.ts'
import { errMessage } from './errors.ts'
import { headSlice, tailSlice } from './text.ts'
import { GOAL_BLOCKED_MIN_ROUNDS, createGoalRecord } from './goal.ts'
import type { GoalRecord } from './goal.ts'
import type { GrindMeter } from './grind.ts'
import { grindState } from './grind.ts'
import { unscreenedNotice, UNSCREENED_PREFIX } from './security-posture.ts'
...
export interface SecurityScreenVerdictInput {
  content: string
  tool: string
  source: string
}
...
export interface ToolContextRef {
  ...
  pendingPrepareNextTurn?: number | undefined
  pendingTransformContext?: number | undefined
  onGapWork?: ((work: GapWork) => void) | undefined
  ...
  screenToolResult?: ((tool: string, result: string, unscreenable: boolean) => Promise<boolean | 'unscreened'>) | undefined
  screenExternalContent?: (input: SecurityScreenVerdictInput) => Promise<SecurityScreenVerdict | undefined>
  toolApprovalGate?: ((tool: string) => boolean) | undefined
}
...
export interface HarnessToolConfigSeed {
...
export function coreToolOptions(config: HarnessToolConfigSeed): CoreToolOptions {
  return {
    ...(config.scratchExecEnabled !== undefined ? { scratchExec: config.scratchExecEnabled } : {}),
    ...(config.sharedOwnerAuthIsolation !== undefined ? { ownerAuthExec: config.sharedOwnerAuthIsolation } : {}),
    ...(config.reachExecEnabled !== undefined ? { reachExec: config.reachExecEnabled } : {}),
    ...(config.signingSecret && config.apiBaseUrl ? { controlTools: true } : {}),
...
export function createPiTools(ref: ToolContextRef, opts?: PiToolsOptions): ToolDefinition[] {
  ...
  const recordResult = async <T extends { content: Array<{ type: string; text?: string }>; details?: unknown }>(
    ...
    if (ref.screenToolResult && !screenExempt) {
      ...
      if (screen === false) {
        result = '[tool output quarantined by security posture]'
    ...
    try {
      ...
      if (e instanceof NeedsApproval) {
        ref.pendingApprovals?.push({
          ...
          kind: e.kind,
          ...(e.matched !== undefined ? { matched: e.matched } : {}),
          ...(params.purpose ? { purpose: params.purpose } : {}),
  ...
  const cron = defineTool({
    ...
    description:
      ...
      "you're added automatically; the group must already exist — post to it once with the surface tool's " +
  ...
  const surfaceToolName = opts?.surfaceName ?? 'surface'
  const surfaceLabel = surfaceToolName === 'web' ? 'Web' : surfaceToolName
  const surface = defineTool({
    name: surfaceToolName,
    label: surfaceToolName,
    description:
    ...
    parameters: Type.Object({
      ...
      text: Type.Optional(
        Type.String({
          description:
            'post/reach: the message to send (the surface renders it). To @-mention, use the surface\'s native mention syntax — person and user-group ids appear in People here / read / search results. A typed `@name` is plain text and pings no one; broadcast pings never fire from text. edit: the new message content.',
        }),
      ),
      ...
      source: Type.Optional(
        Type.Union([Type.Literal('mirror'), Type.Literal('live')], {
          description:
            ...
            '`live` runs the platform\'s own full-history search AS THE ASKING PERSON, via their connected login — it sees ' +
            'exactly what they can see, including their own DMs and private channels (never anyone else\'s). Retry with ' +
            '`live` whenever the mirror comes up empty; if the asker hasn\'t connected their account, the result says where they ' +
    ...
    async execute(callId, params) {
      ...
      const missing = (field: string) =>
        recordResult(
          callId,
          { tool: surfaceToolName, action: params.action, error: `missing_${field}` },
      ...
      switch (params.action) {
        case 'post': {
          ...
          if (params.channel !== undefined || params.recipient !== undefined || params.participants !== undefined) {
            return recordResult(
              callId,
              { tool: surfaceToolName, action: 'post', error: 'post_cannot_address' },
              text(
        ...
        case 'reach': {
          ...
          if (selectors !== 1) {
            return recordResult(
              callId,
              { tool: surfaceToolName, action: 'reach', error: 'reach_target' },
              text(
        ...
        case 'react': {
          ...
          return recordResult(
            callId,
            { tool: surfaceToolName, action: 'react', ok: r.ok, ...(r.deliveryId ? { deliveryId: r.deliveryId } : {}) },
        ...
        case 'edit': {
          ...
          return recordResult(
            callId,
            { tool: surfaceToolName, action: 'edit', ok: r.ok, ...(r.deliveryId ? { deliveryId: r.deliveryId } : {}) },
        ...
        case 'delete': {
          if (params.ref === undefined) return missing('ref')
          await recordCall(callId, { tool: surfaceToolName, action: 'delete', ref: params.ref, ...routing })
          const r = await tc.delete({ ref: params.ref, ...routing })
          return recordResult(
            callId,
            { tool: surfaceToolName, action: 'delete', ok: r.ok, ...(r.deliveryId ? { deliveryId: r.deliveryId } : {}) },
        ...
        case 'read_thread': {
          ...
          if (!r.ok)
            return recordResult(
              callId,
              { tool: surfaceToolName, action: 'read_thread', ok: false },
          ...
          return recordExternalResult(
            callId,
            { tool: surfaceToolName, action: 'read_thread', ok: true, count: messages.length },
        ...
        case 'whats_new': {
          ...
          if (!r.ok)
            return recordResult(
              callId,
              { tool: surfaceToolName, action: 'whats_new', ok: false },
        ...
        case 'search': {
          ...
          if (!r.ok)
            return recordResult(
              callId,
              { tool: surfaceToolName, action: 'search', ok: false },
          ...
          if (hits.length) {
            ...
            body = `[no matches in the platform's full history as far as the asking person can see — a message in someone else's DMs or private channels stays invisible; ask them to forward it]`
          } else if (r.coverageSince) {
            body = `[no matches in the mirror — it only covers this channel since ${r.coverageSince}; retry with source "live" to search full history as the asking person]`
          }
          return recordExternalResult(
            callId,
            {
        ...
        case 'read_members': {
          await recordCall(callId, { tool: surfaceToolName, action: 'read_members' })
          const r = await tc.readMembers()
          if (!r.ok)
            return recordResult(
              callId,
              { tool: surfaceToolName, action: 'read_members', ok: false },
          ...
          return recordResult(
            callId,
            { tool: surfaceToolName, action: 'read_members', ok: true, count: members.length },
            text(body),
          )
        }
        case 'read_file': {
          if (params.ref === undefined) return missing('ref')
          await recordCall(callId, { tool: surfaceToolName, action: 'read_file', ref: params.ref })
          const r = await tc.readFile(params.ref)
          if (!r.ok)
            return recordResult(
              callId,
              { tool: surfaceToolName, action: 'read_file', ok: false },
        ...
        default:
          return recordResult(
            callId,
            { tool: surfaceToolName, error: 'unknown_action' },
            text(
      ...
      try {
        ...
        if (error instanceof NeedsApproval) {
          ref.pendingApprovals?.push({
            ...
            kind: error.kind,
            ...(error.matched !== undefined ? { matched: error.matched } : {}),
