...
import {
  ...
  SessionManager,
  type AgentSession,
} from '@qm/pi-coding-agent'
import { InMemoryCredentialStore, type Api, type Model } from '@earendil-works/pi-ai'
import type { ConversationTurn, ScopeId, SessionEntry } from '@qm/types'
import type {
  GapPhase,
  GapPhases,
  GapWork,
...
import type { ProviderKeys } from '@qm/types'
import type { HarnessDetectInput, HarnessDetectResult, HarnessCompactInput } from '@qm/types'
import type { HarnessTurnInput, HarnessTurnResult } from '@qm/types'
import { NonRetryableTurnError } from './turn-error.ts'
import { MAX_LLM_REQUEST_BYTES } from './attachments.ts'
import { sleep } from './async.ts'
import { swallow, swallowAs, errMessage } from './errors.ts'
import {
...
import { defineHarness, envelopeWithoutMessages, type Harness } from './define-harness.ts'
import { createPiTools, pauseStampAfterToolCall, type ToolContextRef } from './pi-tools.ts'
import type { McpToolDescriptor } from '@qm/types'
import { startSignalPoll, type RunSignalStore } from './run-signal-store.ts'
import {
  planColdStartSeed,
...
import { parseSecurityScreenVerdict, SECURITY_SCREEN_STEP, SECURITY_SCREEN_SYSTEM_PROMPT } from './security-posture.ts'
...
export function emptyEndingNote(opts: {
  ...
  pollFire: boolean
  ref: {
    silentRequested?: boolean | undefined
...
export type OutputBudgetGuardResult =
  | { kind: 'ok' }
  | { kind: 'raised'; from: number; to: number; estimatedPromptTokens: number }
...
export function createPiHarness(opts?: PiHarnessOptions): Harness {
  ...
  const defaultTurnWallClockMs = opts?.turnWallClockMs ?? 0
    ...
    },
    {
      async runTurn(turn: HarnessTurnInput): Promise<HarnessTurnResult> {
        ...
        try {
          ...
          entry.ref.current = turn.tools ?? null
