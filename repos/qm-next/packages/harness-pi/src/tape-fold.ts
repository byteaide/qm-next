import type { TapeRecord } from '@qm/types'
...
function contextEvent(row: TapeRecord): TapeEvent | null {
  ...
  if (p?.event === 'compaction' || p?.event === 'legacy_import' || p?.event === 'legacy_patch' || p?.event === 'interrupt') {
  ...
  for (const row of rows) {
    ...
    if (ev) {
      if (ev.event === 'legacy_import') {
        ...
        f.boundaries = cut
          ? f.boundaries.filter((b) => b.pos >= cut.pos).map((b) => ({ pos: b.pos - cut.pos + 1, entrySeq: b.entrySeq }))
