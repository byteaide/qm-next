import type { ConversationTurn, OverheardEntryPayload, ScopeId, SessionEntry } from '@qm/types'
import { contextSummaryPayload, forModelContext, INTERRUPTED_TOOL_RESULT } from './context-compaction.ts'
import { isoFromTs, messageTag } from './message-tag.ts'
...
export function selectOverheardToImport(
  ...
  const fresh = incoming
    ...
    .sort((a, b) => {
      const at = a.ts as string
      const bt = b.ts as string
      if (at < bt) return -1
      if (at > bt) return 1
      return 0
    })
...
type ReplayTextBlock = { type: 'text'; text: string }
type ReplayToolCallBlock = { type: 'toolCall'; id: string; name: string; arguments: Record<string, unknown> }
type ReplayZeroUsage = {
  input: number
  output: number
  cacheRead: number
  cacheWrite: number
...
export type ReplayMessage =
  | { role: 'user'; content: ReplayTextBlock[]; timestamp: number }
  | {
      role: 'assistant'
      content: Array<ReplayTextBlock | ReplayToolCallBlock>
      timestamp: number
      stopReason: 'stop'
      usage: ReplayZeroUsage
    }
  | {
      role: 'toolResult'
      toolCallId: string
      toolName: string
      content: ReplayTextBlock[]
      isError: boolean
      timestamp: number
    }

export type PiReplayMessage = ReplayMessage

export function zeroUsage(): ReplayZeroUsage {
  return {
    input: 0,
    output: 0,
    cacheRead: 0,
...
const hasToolCall = (m: ReplayMessage): boolean =>
  m.role === 'assistant' && m.content.some((c) => c.type === 'toolCall')

export function reconstructMessagesFromHistory(history: readonly SessionEntry[]): ReplayMessage[] {
  ...
  const raw: ReplayMessage[] = []
  const userMsg = (text: string, ts: number): ReplayMessage => ({
    role: 'user',
    content: [{ type: 'text', text }],
    timestamp: ts,
  })
  const asstText = (text: string, ts: number): ReplayMessage => ({
    role: 'assistant',
    content: [{ type: 'text', text }],
    timestamp: ts,
    stopReason: 'stop',
...
export function planColdStartSeed(
  reconstructed: readonly ReplayMessage[] | null,
