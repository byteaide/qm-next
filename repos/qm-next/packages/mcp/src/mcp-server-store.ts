 ...
 * connector credentials — reachable only through core, never injected into
 * sandboxes.
 *
 * qm parity port: `repos/qm/src/mcp/mcp-server-store.ts`. The store is a
 * thin wrapper over a `DurableMap<McpServer>` so the same code paths back
 * both the in-memory dev profile and the Postgres production profile.
 */
