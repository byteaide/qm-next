 ...
 *
 * qm parity port: `repos/qm/src/mcp/mcp-client.ts`. Adapts the qm
 * `McpFetch`/`McpAuth` shapes to qm-next's no-types-namespace pattern
 * and uses ES modules (qm is also ES module via tsx, but the parity is
 * a 1:1 translation — both speak JSON-RPC over HTTP and cache
