/**
 * Skill pack git fetcher, ported from qm's `skills/pack-fetcher.ts`:
 * validates the repo URL (https, no creds, public IP), checks out the
 * requested ref into a temp dir, and returns the working-tree files.
 * Authentication is provided structurally — callers plug in
 * `serviceCredential`/`connectorToken` lookups so secrets never enter
 * the command line.
 */
...
export function createGitFetcher(opts: GitFetcherOptions = {}): SkillPackFetcher {
  ...
  async function git(
    ...
    try {
      ...
      throw new Error(scrub(((e as { stderr?: string }).stderr) ?? (e instanceof Error ? e.message : String(e)), auth), {
        cause: e,
      })
    }
  }
