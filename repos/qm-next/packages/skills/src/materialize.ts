/**
 * Skill materialization, ported from qm's `skills/materialize.ts`: writes
 * the `skills/` tree a sandbox sees (`SKILL.md` per skill, the optional
 * per-skill files, pack shared files under `skills/.packs/<id>/`, and the
 * `.index`/`.tree` markers). The same content hash on the index/tree
 * marker means we skip the write; mismatched hashes mean we reconcile
 * stale paths before laying the new tree.
 */
import { createHash } from 'node:crypto'
import type { Sandbox, SandboxHandle } from '@qm/types'
import type { SkillBundle } from './skill-bundle-store.ts'
import type { SkillFile, SkillRecord, SkillResolution } from './contract.ts'
import { safeSkillDirName, safeSkillFilePath } from './manifest.ts'
...
import { createKeyedQueue, swallow } from './util.ts'

const SKILL_PACKS_DIR = `${SKILLS_DIR}/.packs`
const INDEX_MARKER = `${SKILLS_DIR}/.index`
const TREE_MARKER = SKILL_TREE_MARKER
...
function indexHash(resolved: SkillResolution[]): string {
  ...
  const entries = resolved
    .filter((r) => r.skill)
    .map((r) => `${r.skill!.name}\0${renderedBody(r.skill!)}`)
    .sort()
...
function treeHash(skill: SkillRecord, bundles: SkillBundle[]): string {
...
function packRoot(skill: SkillRecord): string | null {
  return skill.pack ? `${SKILL_PACKS_DIR}/${safeSkillDirName(skill.pack.packId)}` : null
}

function renderedBody(skill: SkillRecord): string {
  const root = packRoot(skill)
  return root
...
async function materializeSkillIndexUnlocked(
  ...
  const names = resolved
    .flatMap((r) => (r.skill ? [safeSkillDirName(r.skill.name)] : []))
    .sort()
  ...
  for (const r of resolved) {
    if (!r.skill) continue
    entries.push({
      path: `${SKILLS_DIR}/${safeSkillDirName(r.skill.name)}/SKILL.md`,
      content: renderedBody(r.skill),
    })
  }
  entries.push({
    path: INDEX_MARKER,
...
async function materializeSkillTreeUnlocked(
  ...
  const dir = `${SKILLS_DIR}/${safeSkillDirName(skill.name)}`
  const marker = `${dir}/${TREE_MARKER}`
  const want = treeHash(skill, bundles)
  ...
  for (const name of index?.names ?? []) {
    if (name === safeSkillDirName(skill.name)) continue
  ...
  const entries: LayEntry[] = [{ path: `${dir}/SKILL.md`, content: renderedBody(skill) }]
...
export function createSkillMaterializer(advisoryLock?: MaterializationLock): SkillMaterializer {
...
export function skillsIndexLine(resolved: SkillResolution[]): string {
