/**
 * Skill path-collision machinery, ported from qm's `skill-collision.ts`.
 * Computes the materialized file set a skill claims, detects overlaps with
 * the claimed path registry, and throws `SkillPackCollisionError` when an
 * import would clobber an existing path.
 */
