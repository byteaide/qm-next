/**
 * Skill pack ingest, ported from qm's `skills/ingest.ts`. A fetched repo is
 * walked for `SKILL.md` files; each is normalized via `normalizeSkill`,
 * filtered by `PackConfig` globs/excludes, and the eligible ones are
 * upserted into the SkillStore as published skills. Bundles are the
 * non-`SKILL.md` files outside any skill directory — they become
 * SkillBundle entries on the bundle store.
 */
import { isOrgEligibleScope, normalizeSkill, type NormalizedSkill, type PackConfig } from './normalize.ts'
import { type UpsertOutcome, upsertSeedSkill } from './seed.ts'
import type { SkillFile, SkillManifest, SkillPackRef, SkillStore } from './contract.ts'
...
export async function importPack(
  ...
  const plan = planIngest(repo, { ...(ctx.pack.config ? { config: ctx.pack.config } : {}), nativeNames: ctx.nativeNames })
  ...
  const toWrite: Array<{ upstreamName: string; manifest: SkillManifest; pack: SkillPackRef }> = []
  for (const c of plan.candidates) {
    ...
    toWrite.push({
      ...
      manifest: { ...c.normalized.manifest, files: assets },
      pack: { packId: ctx.pack.id, commit: repo.commit, upstreamName: c.upstreamName },
    })
  }
  ...
  for (const { manifest, pack } of toWrite) {
