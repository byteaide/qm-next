/**
 * Skill frontmatter normalization, ported from qm's `normalize.ts`: turns a
 * raw SKILL.md into a SkillManifest, derives `requiredCapabilities` from
 * `egress` or `requiredCapabilities`, recognizes the `private`/`agent_only`
 * flags, and flags skill-scope eligibility via `isOrgEligibleScope`.
 */
...
export function normalizeSkill(
  ...
  const safeFiles = files && files.length ? files.map((f) => ({ ...f, path: safeSkillFilePath(f.path) })) : undefined

  return {
    manifest: { name, description, requiredCapabilities, body, ...(safeFiles ? { files: safeFiles } : {}) },
    ...(scopeHint ? { scopeHint } : {}),
    private: isPrivate,
    declaredCreds,
    meta,
