...
import { createKeyedQueue, sleep, swallowAs } from './util.ts'
...
export function createLocalSandbox(opts: LocalSandboxOptions = {}): Sandbox {
  ...
  async function runContainer(name: string, scope: string | undefined, withVolume: boolean): Promise<void> {
    ...
    const args = [
      ...
      ...(orgLabel ? ['--label', `qm.org=${orgLabel}`] : []),
      '--label',
      'agent_env=dev',
      '--network',
      net,
  ...
  const sandbox: Sandbox = {
    ...
    async provision(layers: WorkspaceLayer[], provOpts?: ProvisionOptions): Promise<SandboxHandle> {
      ...
      try {
        ...
        const roData = opts.layerData ? await opts.layerData(layers) : []
