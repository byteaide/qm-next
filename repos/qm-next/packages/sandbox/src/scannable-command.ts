/**
 * Shell-semantics command normalizer (qm `src/policy/command-policy.ts`
 * scannableCommand parity, X3b full). Policy rules run against the
 * *scannable* form of a command rather than its literal text: quoted
 * data is stripped, bare words are unquoted so quoting cannot evade
 * word-boundary rules, written heredocs are emptied, and payloads the
 * shell would actually execute are appended as extra scan lines —
 * `sh -c`/`eval` payloads, `env -S`, wrapper commands (sudo, nice,
 * timeout, xargs, nohup, stdbuf, coproc), pipelines whose consumer
 * executes stdin (`printf ... | bash`), herestrings (`bash <<< 'rm'`),
 * simple variable indirection (`r=rm; "$r" -rf`), and SQL client
 * payloads (`psql -c "DROP TABLE ..."`). Recursion is capped at depth 8.
 */
