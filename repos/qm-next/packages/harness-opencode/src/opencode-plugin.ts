...
function requiredEnv(
  name: 'OPENCODE_BRIDGE_URL' | 'OPENCODE_BRIDGE_SECRET',
): string {
...
function jsonSchemaToZod(z: any, schema: JsonSchema): any {
  ...
  if (schema.const !== undefined)
    return schemaWithMetadata(schema, literal(z, schema.const))
  if (schema.enum)
    return schemaWithMetadata(
      schema,
      union(
  ...
  if (Array.isArray(schema.type)) {
    const { description: _description, default: _default, ...rest } = schema
    return schemaWithMetadata(
      schema,
      union(
        z,
...
export function needsHistoryImport(
  messages: OpenCodeWithParts[],
  history: OpenCodeWithParts[] | undefined,
): boolean {
...
const OpenCodeBridgePlugin: Plugin = async ({ client }) => {
  ...
  const request = async <T>(path: string, init?: RequestInit): Promise<T> => {
    ...
    if (!res.ok)
      throw new Error(`OpenCode bridge ${path} failed: ${res.status} ${text}`)
    return JSON.parse(text) as T
  }

  const discovered = await request<BridgeTool[] | { tools: BridgeTool[] }>(
    'definitions',
  )
    ...
    bridgeTools.map((definition) => [
      ...
      tool({
        ...
        async execute(args: Record<string, unknown>, context: ToolContext) {
          ...
          if (!callID)
            throw new Error('OpenCode omitted the bridged tool call ID')
          const result = await request<ToolResponse>(
            `session/${encodeURIComponent(context.sessionID)}/tool`,
            {
              method: 'POST',
              body: JSON.stringify({
          ...
          if (result.terminate) {
            await client.session
              .abort({ path: { id: context.sessionID } })
              .catch(() => undefined)
          }
  ...
  return {
    ...
    'experimental.chat.messages.transform': async (
      _input: object,
      output: { messages: OpenCodeWithParts[] },
    ) => {
      const currentLastUser = output.messages.findLast(
