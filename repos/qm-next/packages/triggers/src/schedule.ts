/**
 * Schedule math ported from qm `src/cron/schedule.ts`: normalization,
 * next-fire recovery (restart-safe), and slot advancement. Calendar
 * schedules evaluate through croner with an IANA timezone; interval
 * schedules are epoch-anchored arithmetic.
 */
import { Cron as Croner } from 'croner'
import type { CronSchedule } from './contract.ts'
...
function finiteEpochMs(value: number | undefined, name: string): number | undefined {
  ...
  if (!Number.isFinite(value) || !Number.isSafeInteger(value)) throw new Error(`${name} must be a finite integer timestamp in ms`)
  return value
}
...
export function normalizeSchedule(
  ...
  if (hasCron) {
    if (typeof input.cron !== 'string' || input.cron.trim() === '') throw new Error('schedule.cron must be a non-empty string')
  ...
  if (input.everyMs !== undefined && input.everyMs >= DAY_MS) {
    throw new Error(
      'schedule.everyMs >= 24h is almost always a clock-time schedule in disguise — it anchors to an ' +
        ...
        'runs (e.g. { cron: "30 7 * * 1-5", timezone: "Asia/Shanghai" }). Reserve everyMs for genuine ' +
        'sub-day polling where wall-clock time does not matter.',
    )
  }

  const everyMs = finitePositiveMs(input.everyMs, 'schedule.everyMs')
  const firstFireAt = finiteEpochMs(input.firstFireAt, 'schedule.firstFireAt') ?? (everyMs !== undefined ? now + everyMs : now)
  const schedule = { ...(everyMs !== undefined ? { everyMs } : {}), firstFireAt }
  return { schedule, nextFireAt: firstFireAt }
}

/** User-input guard: recurring intervals below one minute are refused. */
...
/**
 * The slot a cron is due at, recomputed from durable inputs alone — a
 * lost `nextFireAt` (legacy row, restart) is recovered instead of skipped.
 */
...
/** The slot following a fire: calendar schedules keep clock-time anchors. */
