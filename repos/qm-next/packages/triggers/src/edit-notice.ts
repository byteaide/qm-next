/**
 * Cron edit notices (qm src/triggers/edit-notice.ts): when someone other
 * than the owner edits a shared cron, the owner hears what changed in
 * their DM. The composer is pure; the sink is injected (delivery enqueue,
 * directory names, optional admin link) so routes adapt qm-next's
 * provider-native destinations.
 */
import { createHash } from 'node:crypto'
import type { CronSchedule, Destination } from '@qm/types'
...
export interface CronEditNoticeSink {
  ...
  directoryMember(principalId: string): Promise<{ displayName?: string } | null>
  /** Resolvable destination for the owner's DM; null skips the notice. */
  ownerDestination(ownerPrincipalId: string): Promise<Destination | null>
  cronAdminUrl?(cron: { id: string; title?: string }): string | undefined
  channelName?(channelId: string): Promise<string | undefined>
}

/**
 * Notify the owner of a third-party edit. `editFingerprint` dedupes
 * repeated identical patches through the delivery idempotency key. Errors
 * never fail the edit — they surface through `onError` (default warn).
 */
export async function notifyOwnerOfCronEdit(
  sink: CronEditNoticeSink,
  args: {
    cron: { id: string; owner: string; title?: string }
    editorId: string
  ...
  try {
    const [editor, destination] = await Promise.all([
      sink.directoryMember(args.editorId).catch(() => null),
      sink.ownerDestination(args.cron.owner),
    ])
    if (!destination) return
    const editKey = createHash('sha256')
      .update(`${args.cron.id}:${args.editorId}:${args.editFingerprint}`)
      .digest('hex')
      .slice(0, 16)
    const url = sink.cronAdminUrl?.(args.cron)
    const label = args.cron.title ?? 'shared'
    let ref = 'shared'
    if (url) ref = `<${url}|${label}>`
    else if (args.cron.title) ref = `"${args.cron.title}"`
    await sink.enqueueDelivery({
      ...
      idempotencyKey: `cron-edit-notice:${args.cron.id}:${editKey}`,
    })
  } catch (e) {
    const message = e instanceof Error ? e.message : String(e)
    ;(onError ?? ((m) => console.warn('[cron-edit-notice]', m)))(message)
  }
}
