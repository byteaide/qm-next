/**
 * Monitor broker (qm `src/monitors/monitor-broker.ts`): arms a watch on
...
export function createMonitorBroker(deps: MonitorBrokerDeps): MonitorBroker {
  ...
  return {
    async watch(processId, opts): Promise<BackgroundWatchResult> {
    ...
    async unwatch(monitorId): Promise<BackgroundUnwatchResult> {
