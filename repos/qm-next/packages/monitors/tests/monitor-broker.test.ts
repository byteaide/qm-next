/**
 * Memory monitor-broker tests (parity 16.0): watch arms a monitor
 * inheriting the turn's owner/scope/thread/destination, a repeat
 * watch reattaches the same record with new settings, watch on an
 * exited job returns the final tail without arming, watch refuses
 * cross-scope or unknown jobs and unsafe regex patterns, and
...
import { createMemoryProcessRegistry } from '@qm/processes'
...
test('broker watch: arms a monitor inheriting the turn owner, scope, thread, and destination', async () => {
...
test('broker watch: same job in the same thread twice reattaches instead of double-arming', async () => {
...
test('broker watch: repeat watch re-arms with the new settings instead of silently keeping stale ones', async () => {
  const { broker, store } = await harness()
...
test('broker watch: refuses a job from another scope and an unknown job', async () => {
...
test('broker watch: on an exited job with output returns its final tail without arming', async () => {
...
test('broker watch: on an exited job with no output returns success without arming', async () => {
...
test('broker watch: rejects invalid regex patterns up front', async () => {
  ...
  await assert.rejects(
    () => broker.watch('p-1', { pattern: '.*a.*a.*a.*a.*a.*a.*a.*a.*b' }),
    /literal alternatives/,
  )
})

test('broker unwatch: removes own monitors only', async () => {
  const { broker, store } = await harness()
