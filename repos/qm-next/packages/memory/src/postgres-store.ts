/**
 * Postgres ScopeMemory: durable twin of the in-process implementation,
 * ported from qm's `postgres-memory-service.ts`. Every mutation takes the
 * per-scope advisory lock (`hashtext('memory'), hashtext(scopeId)`) inside
 * a transaction, so concurrent writers serialize and `replaceIfRevision`
 * is a true compare-and-swap on the head sequence.
 */
import type { ScopeId } from '@qm/types'
import { createPgPool, withPgTransaction, type PgPool } from '@qm/store'
...
export function createPostgresScopeMemory(connectionString: string): ScopeMemory {
  const pool: PgPool = createPgPool(connectionString, MEMORY_SCHEMA_STATEMENTS)

  async function currentHead(scopeId: ScopeId): Promise<{ body: string; seq: number; at?: number }> {
    const rows = await pool.q('SELECT body, seq, at FROM memory_revisions WHERE scope_id = $1 ORDER BY seq DESC LIMIT 1', [
      scopeId,
    ])
    return rows[0]
      ? { body: String(rows[0].body ?? ''), seq: Number(rows[0].seq), ...(rows[0].at != null ? { at: Number(rows[0].at) } : {}) }
      : { body: '', seq: 0 }
  }
  ...
  async function conditionalReplace(
    ...
    const client = await (await pool.pool()).connect()
    try {
      await client.query('BEGIN')
      await client.query("SELECT pg_advisory_xact_lock(hashtext('memory'), hashtext($1))", [scopeId])
      const head = await client.query('SELECT body, seq FROM memory_revisions WHERE scope_id = $1 ORDER BY seq DESC LIMIT 1', [
        scopeId,
      ])
      ...
      if (next !== String(head.rows[0]?.body ?? '')) {
        await client.query('INSERT INTO memory_revisions (scope_id, seq, op, body, author, at) VALUES ($1, $2, $3, $4, $5, $6)', [
          scopeId,
          seq + 1,
          op,
          next,
  ...
  return {
    ...
    get: async (scopeId) => (await currentHead(scopeId)).body,
    ...
    append: async (scopeId, facts, at, author) => {
      ...
      const probe = foldCapture(await currentHead(scopeId).then((h) => h.body), facts, at, trustedProvenance)
    ...
    recall: async (scopeId, opts) =>
      recallBody(await currentHead(scopeId).then((h) => h.body), opts?.maxChars ?? MEMORY_RECALL_MAX_CHARS),

    query: async (scopeId, q, limit = 20) => queryBullets(await currentHead(scopeId).then((h) => h.body), q, limit),
    ...
    restore: async (scopeId, revision, expectedRevision, author) => {
      ...
      const rows = await pool.q('SELECT body FROM memory_revisions WHERE scope_id = $1 AND seq = $2', [
        scopeId,
        Number(revision),
      ])
    ...
    updatedAt: async (scopeId) => (await currentHead(scopeId)).at,
    ...
    metadata: async () => {
      ...
      for (const r of rows) {
        ...
        out.set(r.scope_id as ScopeId, { bytes, ...(r.at != null ? { updatedAt: Number(r.at) } : {}) })
      }
      return out
    },
