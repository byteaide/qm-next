...
/**
 * Procedural memory backed by the Memorable CLI, exposed as an ordinary
 * ScopeMemory provider.
 *
 * Recall shells out to `memorable inject` with the turn's task; capture
 * ignores the extracted facts and instead derives a tool-call trace from
 * the session that produced the turn, then relays it through
 * `memorable record`. Everything else (query, get, replace) is a no-op:
 * procedures are not a notebook and are never surfaced for direct editing.
 */
export function createMemorableMemoryProvider(deps: MemorableProviderDeps): ScopeMemory {
  ...
  return {
    ...
    async capture(scopeId: ScopeId, _facts, _at, _author, context?: MemoryCaptureContext) {
    ...
    async recall(scopeId: ScopeId, recOpts?: { maxChars?: number; query?: string }) {
      const task = recOpts?.query?.trim()
      if (!task) return ''
