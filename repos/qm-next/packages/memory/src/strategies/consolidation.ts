/**
 * Memory consolidation, ported from qm's `strategies/consolidation.ts`:
 * after N captures a one-shot model revises the notebook (UPDATE/DELETE/ADD
 * actions) under a compare-and-set write; a consolidating wrapper triggers
 * maintenance after each capture.
 */
...
import type { MemoryModel } from '../strategy.ts'
...
export function createConsolidator(deps: {
  ...
  async function maintain(scopeId: ScopeId): Promise<void> {
    if (degraded.has(scopeId) || !deps.model.oneShot) return
    ...
    try {
      out = await deps.model.oneShot(MEMORY_CONSOLIDATION_PROMPT, numbered)
    } catch {
      return
    }
    const at = now()
    ...
    const after = await deps.memory.get(scopeId)
  ...
  return {
    ...
    async maybeMaintain(scopeId) {
      ...
      if (bulletsBelowMarker(await deps.memory.get(scopeId)) >= afterN) await maintain(scopeId)
    },
  }
}
...
async function captureThrough(
  base: ScopeMemory,
  s: ScopeId,
  facts: string[],
  at: number,
  author: string | undefined,
  context: Parameters<NonNullable<ScopeMemory['capture']>>[4],
): Promise<number> {
  if (base.capture) return base.capture(s, facts, at, author, context)
  return base.append(s, facts, at, author)
}
