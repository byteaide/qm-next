/**
 * Security posture (qm `src/security/security-posture.ts`): policy
 * resolution (`dangerous`/`auto`/`strict`), composition rules across
 * org-floor and scope-overrides, the screening rubric + output
 * contract, the verdict parser, the payload assembler for screening
 * inputs, and the rendered policy prompt the model sees.
 */
...
interface SecurityScreenInput {
  ...
  overheard?: readonly OverheardLike[]
  externalPromptData?: readonly { source: string; content: string }[]
}
...
export function securityScreenPayload(input: SecurityScreenInput): SecurityScreenPayload | null {
  ...
  for (const message of input.overheard ?? []) {
    if (message.role === 'user' && typeof message.text === 'string' && message.text.trim()) {
      payloads.push({
        source: `overheard:${typeof message.name === 'string' ? message.name : 'participant'}`,
