/**
 * Security screener proxy (qm `src/security/security-screener.ts`):
 * chunks the payload by char-boundaries (with overlap), sends the
 * chunks concurrently with a 429-driven retry, and reduces the
 * per-chunk verdicts to a single classification. Shadows run with
 * an in-flight cap so they cannot starve the authoritative calls.
 */
...
async function cancelResponseBody(response: Response): Promise<void> {
  await response.body?.cancel().catch((error) => errMessage(error))
}
  ...
  try {
    for (;;) {
      ...
      if (total > MAX_SECURITY_SCREEN_RESPONSE_BYTES) {
        await reader.cancel().catch((error) => errMessage(error))
        throw new Error('Security screen proxy response exceeds the supported limit')
      }
