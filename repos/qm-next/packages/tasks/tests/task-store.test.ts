/**
 * Memory task store tests (parity 16.0): creates every supported status,
 * filters open tasks by session/origin run, compare-and-set transitions
 * append events only on success, and returned objects are immutable.
 */
import assert from 'node:assert/strict'
import { test } from 'node:test'
import { createMemoryTaskStore } from '../src/index.ts'
import { TASK_STATUSES } from '../src/index.ts'

function store() {
  let time = 1_000
  return createMemoryTaskStore({ now: () => ++time })
}

test('memory task store: creates every supported status and records an initial event', async () => {
  const tasks = store()
  for (const status of TASK_STATUSES) {
    const task = await tasks.create({
      sessionId: 'session',
      originRunId: `run-${status}`,
      title: status,
      status,
    })
...
test('memory task store: lists open tasks filtered by session and origin run', async () => {
  const tasks = store()
...
test('memory task store: uses compare-and-set transitions and appends only successful events', async () => {
  const tasks = store()
...
test('memory task store: returned copies cannot mutate task or event state', async () => {
  const tasks = store()
