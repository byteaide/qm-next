/**
 * Postgres task store (qm `src/tasks/postgres-task-store.ts`):
 * durable per-session tasks with a transition-guarded `UPDATE … WHERE
 * status = $expected` and an append-only `task_events` log written in
...
export interface PostgresTaskStore extends TaskStore {
  close(): Promise<void>
}

export function createPostgresTaskStore(connectionString: string, opts: { now?: () => number } = {}): PostgresTaskStore {
  const now = opts.now ?? (() => Date.now())
  const pg: PgPool = createPgPool(connectionString, SCHEMA)
  const q = pg.q
