/**
 * Task store contract (qm `src/tasks/task-store.ts`): per-session tasks
 * with a status state machine (`pending → in_progress → completed |
 * skipped | failed`) and an append-only event log of transitions.
 */
