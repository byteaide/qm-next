/**
 * Canonical markdown → Slack mrkdwn, ported from qm `src/slack/mrkdwn.ts`
 * (format pipeline of the reference implementation). Code spans, links and
 * tables are stashed so their punctuation survives the rewrite passes;
 * mass mentions are neutralized so replies cannot ping @here/@channel.
 */
