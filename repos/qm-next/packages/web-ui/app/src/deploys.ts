...
function versionLabel(d: DeploymentView): string {
  ...
  if (d.appliedVersion !== undefined && d.appliedVersion !== d.currentVersion)
    return t("v{applied} live · v{current} pending", {
      applied: String(d.appliedVersion),
      current: String(d.currentVersion),
    });
...
function deployedLabel(d: DeploymentView): string {
  ...
  return at ? t("Deployed {when}", { when: relTime(at) }) : t("Deployment time unavailable");
}
...
function ownerLabel(d: DeploymentView): string {
  ...
  return d.createdBy
    ? t("Shared context · created by {creator}", { creator: friendlyPrincipal(d.createdBy) })
    : t("Shared context");
}
    ...
    html`
      ...
      ${listPageTpl({
        ...
        controls: html`<label class="deploy-sort"
          ><span>${t("Sort")}</span>${fieldSelect({
            compact: true,
            ariaLabel: t("Sort apps"),
            value: deploySort,
            onChange: (value) => {
            ...
            options: [
              html`<option value="newest">${t("Newest")}</option>`,
              html`<option value="name">${t("Name")}</option>`,
              html`<option value="status">${t("Status")}</option>`,
            ],
          })}</label
        >`,
        search: {
...
async function commitEditDeploy(d: DeploymentView): Promise<void> {
  ...
  try {
    ...
    if (currentDeployActionView(d.id) === "target") {
      ...
      deployToast = { deployment: updated, text: `${deploymentTitle(updated)}${t(" settings saved.")}` };
...
async function archiveDeploy(d: DeploymentView): Promise<void> {
  ...
  try {
    ...
    deployToast = {
      ...
      text: `${deploymentTitle(d)}${t("is offline and archived.")}`,
      undo: true,
    };
    await refreshDeployments();
...
async function restoreDeploy(d: DeploymentView): Promise<void> {
  ...
  try {
    ...
    deployToast = { deployment: restoredResponse, text: `${deploymentTitle(d)}${t("is restored and running.")}` };
...
function deployWithAgent(): void {
  ...
  conv.newChat();
  conv.composer.state.draft = t("Deploy an app for me. ");
