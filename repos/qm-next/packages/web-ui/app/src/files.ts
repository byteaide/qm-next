...
async function uploadFiles(files: globalThis.File[]): Promise<void> {
  ...
  filesUploading = true;
  filesNotice = picked.length === 1 ? t("Uploading 1 file…") : t("Uploading {n} files…", { n: picked.length });
  drawFiles();
  let uploaded = 0;
  try {
    ...
    filesNotice = picked.length === 1 ? t("Uploaded 1 file.") : t("Uploaded {n} files.", { n: picked.length });
    await loadFiles(appState.viewRenderSeq);
  } catch (e) {
    filesNotice = `${uploaded ? `${t("Uploaded {a} of {b}.", { a: uploaded, b: picked.length })} ` : ""}${errMessage(e, t("Upload failed."))}`;
