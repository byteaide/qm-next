        ...
        <div class="kc-resource-copy">
          ...
          <div class="kc-credential-facts">
            <div class="kc-audit-line">
              ${icon(Activity, 14)}${lastUse ? html`${t("Last used {date} in {scope} · {status}", { date: fmtDate(lastUse.ts), scope: scopeName(lastUse.scopeLabel), status: lastUse.status })}` : t("No audited use yet")}
            </div>
              ...
              ${asks.map(
                (ask) =>
                  html`<div class="kc-access-row">
                    <div>
                      <strong>${scopeName(ask.requesterScopeId)}</strong> ${t("requested {mode} access", { mode: modeLabel(ask.requestedMode) })}
          ...
          ? html`<div class="kc-access-block">
              ...
              ${grants.map(
                (grant) =>
                  html` <div class="kc-access-row">
                    <div>
                      <strong>${scopeName(grant.audienceScopeId)}</strong> · ${modeLabel(grant.mode)}
...
function scopeName(scope: string): string {
  ...
  switch (kind) {
    ...
    case "channel":
      return ref ? t("a Slack channel ({ref})", { ref }) : t("a Slack channel");
    case "group":
      return t("a group DM");
    case "team":
      return ref ? t("a team ({ref})", { ref }) : t("a team");
    case "org":
...
function modeLabel(mode?: string): string {
  return mode === "standing" ? t("standing") : t("one-time");
}
      ...
      secureDropUrl
        ? html`
            <div class="kc-success" role="status">
              <strong>${t("Your one-time page is ready")}</strong><span>${t("Open it in a new tab and paste the secret there.")}</span>
            </div>
...
async function deleteCredential(credential: KeychainCredential): Promise<void> {
  ...
  const impact = active.length
    ? ` ${t(
        active.length === 1
          ? "It will immediately revoke {n} active grant: {scopes}."
          : "It will immediately revoke {n} active grants: {scopes}.",
        { n: active.length, scopes: active.map((grant) => scopeName(grant.audienceScopeId)).join(", ") },
      )}`
    : "";
...
async function revokeGrant(grant: KeychainGrant): Promise<void> {
  ...
  confirmation = {
    ...
    body: t("This {mode} access ends immediately. Automations using it may stop working.", { mode: modeLabel(grant.mode) }),
