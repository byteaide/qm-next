/**
 * Scoped event sinks (qm `src/admin/scoped-event-sink.ts`): the shared
 * machinery behind the metrics/error/credential-usage/egress sinks — a
 * bounded in-memory ring and a Postgres sink over @qm/store's PgPool with
 * fire-and-forget writes, settle-before-read and declared column schemas.
 */
...
export function createScopedEventSink<E extends ScopedEvent, In>(opts: ScopedEventSinkOptions<E, In>): ScopedEventSink<E, In> {
  const events: E[] = []
  return {
    record(input) {
