/**
 * Retention report (qm `src/admin/retention.ts`): DAU/WAU/MAU stickiness,
 * 30d new-vs-returning, weekly cohorts and per-user percentiles, computed
 * per-request from attributed turns.
 */
import { forEachAttributedTurn, type AttributedTurn, type ParticipantWindow } from './attribution.ts'

const DAY = 86_400_000

export interface RetentionInput {
  sessions: { id: string; type: string; scopeId: string; createdAt: number }[]
