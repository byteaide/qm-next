/**
 * Background exec broker (qm `src/connectors/background-exec-broker.ts`):
 * start a background process inside a sandboxed handle, register it
 * with the process registry, poll for early output, and surface
 * read/write/stop/list via the ProcessSandbox contract. Reattaches to
 * an existing running job when the redacted command matches.
 */
import {
  awaitProcessExit,
  pollProcess,
...
const DEFAULT_TTL_MAX_MS = 24 * 60 * 60_000
...
export function createBackgroundBroker(deps: BackgroundExecBrokerDeps): BackgroundExecBroker {
  ...
  return {
    async start(handle, command, ttlMs): Promise<BackgroundStartResult> {
      ...
      const started = await deps.sandbox.startProcess(handle, command, { env: { PYTHONUNBUFFERED: '1' } })
      const startedId = started?.processId
      if (!startedId) throw new Error('sandbox.startProcess returned no processId')
      processId = startedId
