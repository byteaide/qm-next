/**
 * OIDC client suite (ported from qm plugins/portal test/oidc.test.ts):
 * PKCE, authorize-url shape, code exchange (client_secret_basic), userinfo,
 * JWKS id-token verification and the principal resolution rules.
 */
import assert from 'node:assert/strict'
import { test } from 'node:test'
import { createHash, generateKeyPairSync } from 'node:crypto'
import { exportJWK, SignJWT } from 'jose'
import { pkcePair, buildAuthorizeUrl, exchangeCode, fetchUserinfo, resolvePrincipal, verifyIdToken, type OidcConfig } from '../src/oidc.ts'
...
test('exchangeCode posts client_secret_basic + PKCE verifier and parses the token', async () => {
  ...
  const stub = (async (url: string | URL | Request, init?: RequestInit) => {
    ...
    return new Response(JSON.stringify({ ok: true, access_token: 'AT', id_token: 'IDT' }), { status: 200 })
  }) as typeof fetch
...
test('exchangeCode treats Slack 200 {ok:false} as an error', async () => {
  const stub = (async () => new Response(JSON.stringify({ ok: false, error: 'invalid_code' }), { status: 200 })) as typeof fetch
...
test('verifyIdToken requires a valid signature, issuer, audience, subject, nonce, and timestamps', async () => {
  ...
  await assert.rejects(verifyIdToken(provider, await multiAudience('other-client'), 'N', fetchJwks), /authorized party/)
...
test('resolvePrincipal claim=sub returns the subject untouched', async () => {
  assert.equal(await resolvePrincipal({ claim: 'sub' }, { sub: 'U1', claims: {}, userinfo: { email: 'a@b.com' } }), 'U1')
})
...
test('resolvePrincipal claim=email requires the verified userinfo response', async () => {
  await assert.rejects(
    resolvePrincipal({ claim: 'email' }, { sub: 'g-123', claims: { email: 'a@acme.com', email_verified: 'true' }, userinfo: {} }),
    /no email/,
  )
})
...
test('resolvePrincipal claim=email rejects missing or unverified emails', async () => {
  ...
  await assert.rejects(resolvePrincipal({ claim: 'email' }, { sub: 'g', claims: {}, userinfo: { email: 'a@b.com' } }), /not verified/)
  await assert.rejects(
    resolvePrincipal({ claim: 'email' }, { sub: 'g', claims: {}, userinfo: { email: 'a@b.com', email_verified: false } }),
    /not verified/,
  )
})
...
test('resolvePrincipal allowedEmailDomain gates the email suffix and the hd claim', async () => {
  ...
  await assert.rejects(resolvePrincipal(rule, { sub: 'g', claims: {}, userinfo: { email: 'a@gmail.com', email_verified: true } }), /permitted domain/)
  await assert.rejects(
    resolvePrincipal(rule, { sub: 'g', claims: {}, userinfo: { email: 'a@example.com', email_verified: true, hd: 'evil.com' } }),
    /permitted domain/,
  )
  await assert.rejects(
    resolvePrincipal(rule, { sub: 'g', claims: {}, userinfo: { email: 'a@notexample.com', email_verified: true } }),
    /permitted domain/,
  )
})
...
test('resolvePrincipal allowedEmails permits only the seeded verified addresses', async () => {
  ...
  assert.equal(
    await resolvePrincipal(rule, { sub: 'g', claims: {}, userinfo: { email: 'admin@example.com', email_verified: true } }),
    'admin@example.com',
  )
  await assert.rejects(
...
test('resolvePrincipal admits an invited external address that the env rules reject', async () => {
  ...
  assert.deepEqual(asked, [], 'an address the env rules permit never consults the invited gate')

  assert.equal(await resolvePrincipal(rule, verified(' Guest@Partner.test '), invited), 'guest@partner.test')
  assert.deepEqual(asked, ['guest@partner.test'], 'the gate is asked with the normalized address')
  assert.equal(
  ...
  await assert.rejects(
    resolvePrincipal({ claim: 'email', allowedEmails: ['admin@example.com'] }, verified('stranger@partner.test'), invited),
    /permitted email list/,
  )

  asked.length = 0
  await assert.rejects(resolvePrincipal(rule, { sub: 'g', claims: {}, userinfo: { email: 'guest@partner.test' } }, invited), /not verified/)
  await assert.rejects(resolvePrincipal(rule, { sub: 'g', claims: {}, userinfo: {} }, invited), /no email/)
  assert.deepEqual(asked, [], 'an unverified or missing email is refused before the gate is consulted')
