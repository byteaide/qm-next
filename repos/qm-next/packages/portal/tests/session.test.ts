/**
 * Portal session suite (ported from qm plugins/portal test/session.test.ts):
 * seal/open round-trips, domain-separated keys, claim validation ladders,
 * cookie serialization and the returnTo sanitizer.
 */
...
test('openSession enforces kind, sub, and expiry', () => {
  const now = Math.floor(Date.now() / 1000)
  const good = seal({ k: 'session', sub: 'U1', org: 'acme', iat: now, exp: now + 60 } satisfies SessionClaims, sessionKey)
  ...
  const expired = seal({ k: 'session', sub: 'U1', org: 'acme', iat: now - 120, exp: now - 60 } satisfies SessionClaims, sessionKey)
...
test('openSession enforces an absolute session lifetime', () => {
  const now = Math.floor(Date.now() / 1000)
  const current = seal(
    { k: 'session', sub: 'U1', org: 'acme', auth: now - 300, iat: now - 10, exp: now + 60 } satisfies SessionClaims,
    sessionKey,
  )
  assert.equal(openSession(current, sessionKey, Date.now(), 'acme', 600)?.sub, 'U1')

  const tooOld = seal(
    { k: 'session', sub: 'U1', org: 'acme', auth: now - 601, iat: now - 10, exp: now + 60 } satisfies SessionClaims,
    sessionKey,
  )
...
test('openTmp enforces kind and required fields', () => {
  const now = Math.floor(Date.now() / 1000)
  const tmp = seal(
    { k: 'tmp', state: 'abc', nonce: 'xyz', pkceVerifier: 'v', returnTo: '/web-ui/', iat: now, exp: now + 600 } satisfies TmpClaims,
    tmpKey,
  )
  const opened = openTmp(tmp, tmpKey, Date.now())
  assert.equal(opened?.state, 'abc')
  assert.equal(opened?.nonce, 'xyz')
  const session = seal({ k: 'session', sub: 'U1', org: 'acme', iat: now, exp: now + 600 } satisfies SessionClaims, tmpKey)
...
test('openImpersonation enforces kind, actor, target, and expiry; key is domain-separated', () => {
  const now = Math.floor(Date.now() / 1000)
  const good = seal(
    { k: 'impersonate', actor: 'U-admin', target: 'alice@acme', org: 'acme', iat: now, exp: now + 3600 } satisfies ImpersonationClaims,
    impersonateKey,
  )
  ...
  const expired = seal(
    { k: 'impersonate', actor: 'U-admin', target: 'alice@acme', org: 'acme', iat: now - 7200, exp: now - 60 } satisfies ImpersonationClaims,
    impersonateKey,
  )
  assert.equal(openImpersonation(expired, impersonateKey, Date.now()), null)

  const session = seal({ k: 'session', sub: 'U-admin', org: 'acme', iat: now, exp: now + 3600 } satisfies SessionClaims, impersonateKey)
  ...
  const noTarget = seal({ k: 'impersonate', actor: 'U-admin', target: '', org: 'acme', iat: now, exp: now + 3600 }, impersonateKey)
...
test('setCookie/clearCookie carry a Domain attribute only when asked', () => {
  assert.match(setCookie('portal_session', 'v', { secure: true, domain: 'agent.example.com' }), /; Domain=agent\.example\.com;/)
