/**
 * OIDC authorization-code + PKCE client (qm `plugins/portal/src/oidc.ts`):
 * authorize-url building, code exchange (client_secret_basic), userinfo
 * fetch, JWKS id-token verification (nonce + authorized-party checks) and
 * the principal resolution rule (sub or verified email with allow-lists).
 */
...
export async function exchangeCode(cfg: OidcConfig, args: { code: string; codeVerifier: string }, fetchImpl: FetchLike = fetch): Promise<TokenResponse> {
...
export async function fetchUserinfo(cfg: OidcConfig, accessToken: string, fetchImpl: FetchLike = fetch): Promise<Record<string, unknown>> {
...
export async function verifyIdToken(cfg: OidcConfig, idToken: string | null, nonce: string, fetchImpl: FetchLike = fetch): Promise<Record<string, unknown>> {
...
export async function resolvePrincipal(rule: PrincipalRule, args: PrincipalArgs, invited: (email: string) => Promise<boolean> = async () => false): Promise<string> {
