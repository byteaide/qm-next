/**
 * Portal session cookies (qm `plugins/portal/src/session.ts`): HMAC-sealed
 * JSON payloads with domain-separated keys for session / tmp / impersonation
 * claims, cookie serialization helpers and the returnTo sanitizer.
 */
...
export function openSession(token: string | null, key: Buffer, now: number, expectedOrg?: string, maxAgeSeconds?: number): SessionClaims | null {
