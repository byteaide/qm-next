/**
 * Admin-login links (qm `plugins/portal/src/admin-login.ts`): operators mint
 * a five-minute single-use token with the qm CLI; the portal validates the
 * sealed `admin-login` claims and renders the confirmation page script.
 */
import { createHash } from 'node:crypto'
import { deriveKey, open } from './session.ts'

export function openAdminLogin(token: string, secret: string, publicUrl: string, nowMs = Date.now()): { email: string; jti: string; expiresAtMs: number } | null {
  if (secret.trim().length < 32 || token.length > 4096) return null
  const claims = open(token, deriveKey(secret, 'portal.admin-login.v1'))
  if (!claims) return null
  const { k, sub, aud, iat, exp, jti } = claims as Record<string, unknown>
