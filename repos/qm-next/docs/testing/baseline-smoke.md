...
> **状态**：v0.1.0 · 2026-09-19 · **Phase 3J 完成**（sandbox policy engine guard + 业务流 27/27 = 100% + 永久 SKIP 2 → 1）
> **目的**：对 `qm-next @qm/api` 做一次 QA-style 功能测试基线，记录测了什么 / 覆盖率多少 / 失败明细，作为代码演进时回归与扩展依据。
>
> **最终结论**：**331 用例（235 路由 + 21 wave2 + 31 业务流 + 11 operator CLI + 5 真机隔离 + 29 sandbox policy）99.7% pass rate**（330/331 PASS · 1 SKIP 永久 🚫）。覆盖用户面 ~89%、管理员面 ~87%、Connectors 100%、**整体 ~95% + 业务流 27/27 = 100%**。Phase 3J 新增 `packages/sandbox/src/policy.ts`（CommandPolicy 评估器）+ `packages/sandbox/src/default-policy.ts`（10 个 catastrophic patterns：rm -rf /, mkfs, dd /dev/sd?, fork bomb, dd /dev/zero, chown/chmod -R /, DROP DB/TABLE/SCHEMA/INDEX, TRUNCATE）+ `LocalSandboxOptions.policy?: CommandPolicy | 'default-denylist'` + `ExecOptions.throwOnPolicy?: boolean` + `scripts/qa-sandbox-policy.ts`（29 用例：14 unit + 3 allowlist + 1 approval + 5 真机 docker + 6 negative path）。完全闭合 §U26.1（engine guard 拒绝 rm -rf /）。剩余 1 SKIP（§U24.2 飞书 IM 真机）。新增 `docs/testing/real-device-coverage.md` 扩展。所有 S29 / S40 / S42 / S43 回归测试自动转 PASS。
