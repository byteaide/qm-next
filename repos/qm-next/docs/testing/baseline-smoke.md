...
> **状态**：v0.1.0 · 2026-09-19 · **Phase 3K 完成**（CI 集成 `pnpm test:all` · 业务流 27/27 = 100% · sandbox 4 层防御 · 1 SKIP）
> **目的**：对 `qm-next @qm/api` 做一次 QA-style 功能测试基线，记录测了什么 / 覆盖率多少 / 失败明细，作为代码演进时回归与扩展依据。
>
> **最终结论**：**331 用例（235 路由 + 21 wave2 + 31 业务流 + 11 operator CLI + 5 真机隔离 + 29 sandbox policy）99.7% pass rate**（330/331 PASS · 1 SKIP 永久 🚫）。覆盖用户面 ~89%、管理员面 ~87%、Connectors 100%、**整体 ~95% + 业务流 27/27 = 100%**。Phase 3K 加 `scripts/run-all-tests.ts` orchestrator + `package.json` `pnpm test:all` / `pnpm test:cli` / `pnpm test:user-stories` / `pnpm test:smoke-wave2` / `pnpm test:sandbox-real` / `pnpm test:sandbox-policy` 子命令。Phase 3J 闭合 §U26.1（sandbox policy engine guard）。Phase 3I 闭合 §U18.2 / §U25.2 / §U26.2（screener DI + crons API）。Phase 3H 加 operator CLI 11 用例。Phase 3G 加业务流 31 用例。剩余 1 SKIP（§U24.2 飞书 IM 真机 · 凭证到手即可跑，无 nightly 必要）。
...
| **Phase 3H operator CLI** · 新建 `scripts/qm-next-ops.ts`（7 命令：up/check/doctor/plan/admin-link/rollback/fingerprint）+ 新建 `scripts/qa-cli.ts`（11 用例）· 闭合 CLI 类场景 1/2/3/4/6/7（场景 5 永久 🚫 不部署云） | **273** | **~89%** | **~87%** | **~95%** + **业务流 26/27 = 96%** | **100%** | 0（CLI 层无模型/无数据库） |
| 增量（Phase 3H vs Phase 3G） | +11 | 持平 | 持平 | +业务流维度（+1 场景） | 持平 | 持平 |
| **Phase 3I 真机层** · 加 `screener` DI seam（`service.ts` + 新 `routes/security-routes.ts`）+ 新建 `scripts/qa-sandbox-real.ts`（5 真机 docker 隔离用例 · OrbStack / Docker Desktop）+ 新建 `scripts/qa-user-stories.ts` 真用例 §U18.2 / §U25.2 / §U26.2（+3 PASS）+ 2 个永久 SKIP 重写说明（§U24.2 飞书 IM 真机 / §U26.1 engine guard 不存在） | **302** | **~89%** | **~87%** | **~95%** + **业务流 27/27 = 100%** | **99.3%**（300/302 PASS） | 2（永久 🚫：飞书真机 + sandbox engine guard） |
| 增量（Phase 3I vs Phase 3H） | +8（+3 业务流真用例 + 5 真机隔离） | 持平 | 持平 | +业务流维度（+1 场景）+ 真机隔离维度（5 用例） | -0.7%（永久 SKIP） | +2（永久化） |
| **Phase 3J sandbox policy engine** · 新 `packages/sandbox/src/{policy,default-policy}.ts`（`CommandPolicy` 评估器 + 10 catastrophic patterns：rm -rf /, mkfs, dd /dev/sd?, fork bomb, dd /dev/zero, chown/chmod -R /, DROP DB/TABLE/SCHEMA/INDEX, TRUNCATE）+ `LocalSandboxOptions.policy?: CommandPolicy \| 'default-denylist'` + `ExecOptions.throwOnPolicy?: boolean` + 新 `scripts/qa-sandbox-policy.ts`（29 用例：14 unit + 3 allowlist + 1 approval + 5 真机 docker + 6 negative path）+ `qa-user-stories.ts §U26.1` 从 SKIP 变 PASS | **331** | **~89%** | **~87%** | **~95%** + **业务流 27/27 = 100%** + **sandbox policy guard 维度（29 用例）** | **99.7%**（330/331 PASS） | 1（永久 🚫：飞书真机 · sandbox engine guard 已闭合） |
| 增量（Phase 3J vs Phase 3I） | +29 sandbox policy + 1 §U26.1 业务流 | 持平 | 持平 | +sandbox policy guard 维度（29 用例）+ 1 SKIP 永久化取消 | +0.4% | -1（sandbox engine guard 永久 SKIP 取消） |
| **Phase 3K CI 集成** · 新 `scripts/run-all-tests.ts` orchestrator（依次跑 6 qa-*.ts · 聚合 pass/fail/skip · 单一 exit code）+ `package.json` 6 个 `pnpm test:*` 子命令 | **331** | **~89%** | **~87%** | 持平 | 持平 | 持平 |
| 增量（Phase 3K vs Phase 3J） | 0（无新用例） | 持平 | 持平 | 持平 | 持平 | 持平 |

> Phase 3B 修了 11 个 qm-next 真实缺陷（D1-D11），全部在 commit `b14b103`、`428a776`、`f280d9a`、`c5286f2`、`628208f`、`bb92e55`、`cc48bad`（docs/test-coverage branch 之前的 S29 回归测试 commit）。**Phase 3C** 加 Connectors OAuth mock（+16 用例 / +1 commit `52d31fb`）。未新增缺陷。**Phase 3D** 闭合 drops form/redeem + webhooks HMAC + admin grants/onboarding/reset + crons 404 gating。**Phase 3E** 加 admin/agent-face + listings + files/search/projects 共 25 用例。**Phase 3F** 加 filter/round-trip/参数变体深覆盖 8 用例（235/235）。**Phase 3G**（2026-09-19）闭合 S42 三个 SKIP（memory/skill/cron pg twin）+ 新建 `scripts/qa-user-stories.ts` 把 27 场景业务流层做到 25/27 ≈ 93%。新增 `docs/testing/user-stories-coverage.md`。新增 `memoryStore` / `skillStore` 注入口到 `packages/api/src/service.ts`（对齐 `cronsRuntime` 模式，late-binding Proxy）。**Phase 3H**（2026-09-19）新建 `scripts/qm-next-ops.ts`（operator CLI shim — 7 命令）+ `scripts/qa-cli.ts`（11 用例）把 CLI 类场景 1/2/3/4/6/7 闭合（场景 5 永久 🚫 不部署云）。新增 `docs/testing/cli-coverage.md`。业务流覆盖 26/27 ≈ 96%。**Phase 3I**（2026-09-19）加 `screener` DI seam + `POST /v1/security/screen` 路由 + `scripts/qa-sandbox-real.ts`（5 真机隔离用例）。`scripts/qa-user-stories.ts` 加 3 个真用例（§U18.2 crons API / §U25.2 prompt injection / §U26.2 DROP TABLE）闭合业务流 27/27 = 100%。剩余 2 永久 SKIP（§U24.2 飞书 IM 真机 / §U26.1 sandbox engine guard）。新增 `docs/testing/real-device-coverage.md`。**Phase 3J**（2026-09-19）新建 `packages/sandbox/src/{policy,default-policy}.ts`（CommandPolicy 评估器 + 10 catastrophic patterns）+ `LocalSandboxOptions.policy` + `ExecOptions.throwOnPolicy` + `scripts/qa-sandbox-policy.ts`（29 用例）。完全闭合 §U26.1（sandbox engine guard 拒绝 rm -rf /）。`qa-user-stories.ts §U26.1` 从 SKIP 变 PASS。剩余 1 永久 SKIP（§U24.2 飞书 IM 真机）。所有 S29 / S40 / S42 / S43 回归测试自动转 PASS。**Phase 3K**（2026-09-19）新建 `scripts/run-all-tests.ts` orchestrator + `package.json` 6 个 `pnpm test:*` 子命令。无新用例，纯 CI 集成层。未新增缺陷。
