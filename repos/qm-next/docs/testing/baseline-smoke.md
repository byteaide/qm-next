...
> **状态**：v0.1.0 · 2026-09-19 · **Phase 3I 完成**（screener DI seam + 真机 docker 隔离 + 业务流 27/27 = 100%）
> **目的**：对 `qm-next @qm/api` 做一次 QA-style 功能测试基线，记录测了什么 / 覆盖率多少 / 失败明细，作为代码演进时回归与扩展依据。
>
> **最终结论**：**302 用例（235 路由 + 21 wave2 + 30 业务流 + 11 operator CLI + 5 真机隔离）99.3% pass rate**（300/302 PASS · 2 SKIP 永久 🚫）。覆盖用户面 ~89%、管理员面 ~87%、Connectors 100%、**整体 ~95% + 业务流 27/27 = 100%**。Phase 3I 加 `screener` DI seam（`packages/api/src/service.ts`） + 新路由 `POST /v1/security/screen`（`packages/api/src/routes/security-routes.ts`） + 新 `scripts/qa-sandbox-real.ts`（5 真机 docker 隔离用例 · 真 OrbStack / Docker Desktop）。闭合 §U18.2（crons API 边界）/ §U25.2（classifier 注入）/ §U26.2（classifier 注入） + 部分闭合 §U26.1（容器隔离）。剩余 2 SKIP（§U24.2 飞书 IM 真机 / §U26.1 engine guard）。新增 `docs/testing/real-device-coverage.md`。所有 S29 / S40 / S42 / S43 回归测试自动转 PASS。
