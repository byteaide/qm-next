...
| **CI 集成（Phase 3K）** | `scripts/run-all-tests.ts` orchestrator + `package.json` 6 个 `pnpm test:*` 子命令（test:all / test:cli / test:user-stories / test:smoke-wave2 / test:sandbox-real / test:sandbox-policy）。无新增用例；聚合报表 + 单一 exit code |
