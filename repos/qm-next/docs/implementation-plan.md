...
- [x] Remove Web/IM successor-Run approval logic. (**resolved by owner decision A, 2026-09-20 — waived from Phase 7 and deferred to a dedicated ADR-0010 continuation-executor slice; the slice has since landed on `feat/approval-continuation` (same date).** Scope finding: the runtime never entered suspend/resume states — `pending_approval` completed as `succeeded` and decision paths re-drove via a follow-up turn (new Run); the store primitives and decision glue existed but no executor backed them. The deferred slice implemented the executor: decision surfaces now route through `applyApprovalDecision` and no successor Run is created; `docs/architecture.md` §7 describes the current behavior.)
...
- [x] Update `docs/architecture.md` from “current vs target” to current target behavior. (done — the doc now describes the post-cutover runtime: durable event log as the only Run event source, observation-only web stream, deleted legacy contracts; the formerly open ADR-0010 approval continuation executor gap is closed by the landed continuation-executor slice (see "Deferred to a dedicated slice" below).)
...
### Deferred to a dedicated slice (owner decision A, 2026-09-20) — IMPLEMENTED

**ADR-0010 completion — approval continuation executor. Landed on
`feat/approval-continuation` (2026-09-20).** Scope as archived: a
`suspendForApproval` RunStore transition (`awaiting_approval` + executor-lease
release + durable Approval Continuation in `deliveryState.pendingApproval`),
a claimable continuation lane in the turn runner (`claimNextContinuation` —
durable discovery, so a restart between approval and resume still resumes
exactly once), `TurnInput.approval` (with `commandRequestId`) forwarded into
`harness.turns.runTurn`, awaiting-state representation in observation
snapshots + wire (`awaiting_approval` status) + IM suspension delivery
(`onSuspension`), and decision surfaces (Web `/api/approvals/:id` + IM card
clicks) re-routed through `applyApprovalDecision` so no successor Run is
created. An unwired executor fails a paused turn closed with
`approval_continuation_unavailable` — a pending approval never completes as
success. Acceptance evidence: `packages/api/tests/approval-continuation-executor.test.ts`
(the Phase 2 resume tests — "approval resumes the saved command point, not a
blind replay", "no successor Run is created", "restart between approval and
resume still resumes exactly once") plus the memory/Postgres contract cases in
