...
| Capability tokens（opt-in 压缩） | qm-verbatim port 默认 legacy envelope：`{orgId, actorId, scopeId, exp, ...}` flat。Operator 显式 `compress-tokens: true`（qm-post-soul lane B, 2026-09-26，parity #59）后，`mintCapabilityToken(claims, secret, orgId, { compress: true })` 切换到压缩 envelope：`{orgId, __qm_cap_compressed_v1: 'gzip1', data: 'gzip1.<base64url-gzip-bytes>'}`；`verifyCapabilityToken` 通过 marker 嗅探 + 透明解压，wire 协议兼容（同 JOSE 签名覆盖对应字节）。`CAPABILITY_COMPRESS_THRESHOLD = 1024` 字节起跳，`CAPABILITY_COMPRESS_CEILING = 32768`（32 KB）拒超限；`CapabilityTokenError` 三码稳态 `compression_oversize` / `not_a_compressed_payload` / `decompression_failed` | parity #59 |
...
- **Git smart-HTTP 后端**（cluster 1 phase 2，`@qm/deploy-runtime/git-store.ts` + `packages/api/src/routes/deployment-git-routes.ts`）：`ApiConfig.deployGit = true` 时，每次 deploy/redeploy 把部署文件 commit 进 per-deployment bare repo（`<deployGitRepoRoot>/<safeName(id)>.git`，SHA 记到 `DeploymentVersion.commit`），`/v1/deployments/:id/git/{info/refs,git-upload-pack,git-receive-pack}` 通过 spawn `git http-backend` CGI 提供 `git clone`/`git push`；凭据走 `deploy-git` audience 的 capability token（`git-url` 路由铸造，支持 Bearer / Basic / `?token=` 三种携带方式——git 客户端无法自定义 header）。`git` 二进制缺失时路由不注册、`git-url` 保持 403 stub。可选 `GitArchiveStore` 把 repo bundle 存入 `DurableByteStore`，新进程可从 archive 恢复 repo。
