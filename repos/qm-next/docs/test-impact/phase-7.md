...
| 2026-09-20 | ADR-0010 continuation-executor slice landed on `feat/approval-continuation`: `suspendForApproval` + `claimNextContinuation` RunStore transitions (memory+PG, awaiting-only decision guards), turn runner suspend flow + continuation lane, `TurnInput.approval`/`HarnessTurnInput.approval` forwarded, `onSuspension` delivery signal, awaiting snapshot/wire projection, Web/IM decisions through `applyApprovalDecision` (no successor Run). New tests: `packages/api/tests/approval-continuation-executor.test.ts` (the deferred acceptance = Phase 2 resume tests), ADR-0010 cases in `packages/store/tests/stores.test.ts`; updated: `approval-continuation.test.ts` seed drives the real suspend transition, web-ui-relay approval ladder, im-bridge card round-trip. `docs/architecture.md` §7 gap description replaced with the current shape | tiger.w |
