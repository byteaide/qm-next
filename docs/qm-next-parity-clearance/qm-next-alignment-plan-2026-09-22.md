...
| 条目 | 现状 | 触发条件 | 性质 |
|---|---|---|---|
| X3a command-policy 引擎差距审计 | qm-next `command-gate.ts` 117 行 + config 94 行 vs qm `command-policy.ts` 911 行 | 随时可做（**建议提前**，它决定 simulate 解锁路径） | 审计报告 |
| X3b command-policy-simulate 501 → 实现 | `admin-routes.ts:238-244` | X3a 完成 + 引擎收敛 | 移植 |
