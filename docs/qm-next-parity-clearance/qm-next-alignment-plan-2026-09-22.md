...
| X3b command-policy-simulate 501 → 实现 | 最小版 ✅ + 扫描器 ✅（2026-09-22：inline policy + 基线回落 + 引擎唤醒 + i 标志/safe-regex + `scannableCommand` 全量移植，qm 语料 24 项全绿，`matched`=命中子串/qm firstMatch 语义）；余每作用域存储/CRUD + 分层（4b）、G6 收敛 + G8 审批联动（4c） | 移植 |
