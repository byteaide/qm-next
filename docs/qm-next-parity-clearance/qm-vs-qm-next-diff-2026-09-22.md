...
| Command policy 模拟 | X3b 全部收口（2026-09-22，4a/4b/4c/6）：4a `scannableCommand` 全量移植（qm 语料全绿）；4b 每作用域存储（memory+PG 双实现+历史表）+ admin CRUD + simulate qm 保真（ruleSource=organization/scope 算术、policySource 字段）+ `composePolicy`/`evaluateCommandWithLayer` + 沙箱 per-scope `policyFor`；4c G6 收敛决策落地（`docs/adr/0019-command-gate-converges-onto-rule-engine.md`：规则引擎收编为 CommandGate 的 `rule-engine` CommandPolicy，argv/rawText → 同一 `evaluateCommandPolicy`，类目策略继续管结构化操作）+ G8 审批链路闭合（`ExecResult.policyVerdict` 携带 decision/ruleId/matched/reason，harness-pi 据此产出审批卡（matched+approvalKey+purpose）或 denied 分支；throwOnPolicy 路径的 `NeedsApproval` 同样携带）；第 6 批 startup 装配收口（composition root 经 `configureProductionCommandPolicy` 装配静态注册表道——`production: true` 且未选策略时启动即拒（ADR-0002 fail-fast），`commandPolicyId` 覆盖 `QM_COMMAND_POLICY`，rule-engine 以 `resolvePolicy` 挂钩 per-scope store 实现两道同源，gate/registry 暴露为 service 成员；4 条 boot 测试：dev 默认、生产无策略拒启、rule-engine+store 收敛评估（allow/deny）、未知 id 拒启） | 🟢 X3a 差距 G1-G8 全闭合 + startup 装配收口 |
...
2. **明确挂起**（connectors/oauth.ts（X1，2026-09-22 拍板挂起：无当前消费者，触发=实际接入第三方 OAuth 服务或 P5 IM providers 开工）、集群 M 多实例（同日拍板暂缓：单实例形态够用，触发=需要第二实例）、5.2 真机 e2e（凭据已在库，差一次人工发消息配合））——均有登记与触发条件；
...
基线侧 qm 自 2026-09-05 起无移动，不产生新差异。第三批（2026-09-22）已收口：X3b 最小版 + createPlayground 存储侧 + X2 核心审计语义。第四批（2026-09-22，同日连续交付）已收口：a 段 scannableCommand 扫描器 + b 段每作用域存储/CRUD/分层 + c 段 G6 收敛决策（ADR-0019）与 G8 审批链路——X3a 审计的差距 G1-G8 至此全部闭合。第五批（2026-09-22）已收口：5a createPlayground turn-attachment 投递全链路（含 bridge 空回复不投递的 qm 对齐）+ 5b X2 portal 密封流（/auth/impersonate + stop + 代理身份换面）。**收尾拍板（2026-09-22）**：Fly/AWS 部署 provider 不移植（publish 转永久有意偏差）；集群 M 多实例暂缓（触发=需要第二实例）；X1 connectors/oauth 挂起（触发=实际接入第三方 OAuth 服务或 P5 IM providers 开工）；codex-device-login + 订阅 OAuth 搁置为有意偏差（代码就绪，不再等凭据）。第六批（2026-09-22，最终切片）已收口：CommandGate 生产 startup 装配（`production: true` 未选策略 → 启动即拒的 fail-fast 不变量 + rule-engine 经 `resolvePolicy` 与 per-scope store 同源收敛）。**剩余实现类工作 = 0**——对齐计划进入纯挂起/外部门控状态，唯一行动项为 5.2 飞书真机静默腿（凭据已在库，差一次人工发消息配合）。X3a 审计（2026-09-22）已完成。
