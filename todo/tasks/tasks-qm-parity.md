...
- [x] 20.0 【C】监控/合规/生产化 ~0.5d（2026-09-15 单日完成：worktree `aa-feature-p5-web-depth`，commits `dd162c1`/`f01af3b` + 收口提交；`test:pg` 终局全绿 **758 测试 754✔/0✖/4 skip exit 0**——此前"挂起"根因=eager DDL 与 DurableMap 暖建并发建表撞 `pg_type` 唯一索引（23505），两路 DDL 收进共享 `withSchemaLock`（schema-init advisory lock）+ boot 失败回滚防 socket 泄漏；连带修复 session 契约 TRUNCATE 需 CASCADE（tasks→sessions FK）、delivery-queue-pg ack 断言补租约过期 sleep；门禁 typecheck/`pnpm test`/check:im/rescope-check/rehearsal:migrate/rehearsal:backup 均绿）
  - [x] 20.1 PG-twin 接线 + 观测面完整化 + 健康检查 + 监控面板占位（2026-09-15）
    - 组合根 durable-by-default sweep（`packages/api/src/service.ts`）：sessions/runs/directory/approvals/crons（含 PG leader lease）按 `databaseUrl` 选 PG twin；keychain creds/grants/asks、`model_credentials`、`custom_model_providers`、`device_flow_cutover(+resets)`、`mcp_servers`、`oauth_flows`、`consent_links`、`browser_sessions`、`webhooks` 接 `createPostgresMap`（qm 同名表，迁移即直拷）；DurableMap 表 boot 暖建（runbook"启动一次落地全量 schema"成立）
    - 缺口 twin 新建：`@qm/im-core` `createPostgresDeliveryQueue`（SKIP LOCKED 认领/TTL 租约/退避/park 与内存版同语义，bridge 按 `databaseUrl` 注入）；`@qm/api` `createPostgresChannelPolicyStore`（qm 同列 `channel_policy`+`_history`，set 即写历史）；`createPostgresFileStore`（`file_artifacts` qm 同列 DDL）+ `@qm/store` `DurableByteStore`（内容寻址 `files/<sha256>`，local FS/memory，S3 延后记偏差）
    - C.3 收口：tasks/acl/process-registry/run-activity/run-signals/replay-dedupe 构造器随 `databaseUrl` 实例化；admin sink 族（metrics/error/credential-usage/egress/audit-log）在 `databaseUrl` 下常开（不再依赖 admin flag）；`@qm/store` `createPgPool` 构造即应用 DDL（schema 所有权=构造时点，runbook 依赖）
    - 观测面：`GET /healthz` 保持 `{ok:true}`；新增 `GET /readyz`（PG ping，down 503/disabled 透传）；`GET /v1/admin/monitoring/summary` 监控面板占位（uptime/库态/队列耐久性/crons/错误与审计计数，admin 鉴权阶梯）
    - 迁移器联动：`deliveries` 改 `DRAINED` drain-check（切换前强制清零，行不携带）；`channel_policy(+history)`/`file_artifacts` 进 ENTITY_COPIES 直拷；`tasks`/`task_events` 依赖 sessions FK 移到 transforms 后拷贝；`acl_grants_version` 回滚顺序修正（trigger 回写）
