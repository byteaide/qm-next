...
- [x] ~~数据迁移已从 non-goal 转正（替身目标隐含）——确认目标库是独立 PG 实例还是复用 qm 实例。~~ 拍板：**独立 PG 实例**（2026-09-15 按建议 A 落档——qm-next 完全脱离 qm 跑通后再切流量，迁移期 qm 不受影响、回滚干净；19.0 schema diff 以 qm 源库 → qm-next 独立目标库直连对比，不引入中间 schema。如需改判复用实例，19.1 开工前提出）
- [x] ~~slack/钉钉/企微确认回填（原 M4 延期项 18.0/19.0/20.0）。~~ 拍板：suspended；v1 只做飞书 + web（2026-09-15）
- [x] ~~切换终态：qm 退役时间表 vs 长期双跑。~~ 拍板：**qm 退役**（沿 PRD Goal 既定"切换演练通过后 qm 可退役"；21.0 灰度双跑是切换手段而非终态，21.4 tag `v1.0.0` 后 qm 进入只读归档；长期双跑机制作为 runbook 里的 fallback 形态保留，非默认路径）
