...
- [ ] Phase P5 多渠道 + 迁移 + 切换：slack 复活 ‖ dingtalk/wecom → 迁移器 → 双跑演练 + tag v1.0.0 ~4d（2026-09-15 重整：IM 多渠道 suspended；18.0 web 端深化完成（SSE/deep-link/portal SSO 真路径/dark mode）；19.0 数据迁移完成（schema diff + migrate-qm.ts + 演练 44/44 PASS + runbook）；剩 20.0 生产化 + 21.0 切换 tag v1.0.0）
