...
**Status:** Completed（`v1.0.0` @ `af813d3`，2026-09-15；tag 链 `p1`/`p2`/`p3`/`p4` + `v1.0.0`；PRD/Tasks Status 已归位）
**Owner:** @wxd
**Tags:** #qm-parity #qm-next #parity
**Estimate:** ~23d (ai:20d test:3d read:2h)；双/三车道并行墙钟 ~3d（2026-09-13 → 2026-09-15）
...
| Language/runtime | TypeScript (strict, ESM, NodeNext)，Node ^22.19 \|\| >=24 |
| Install | `cd repos/qm-next && pnpm install`（并行阶段开始前由主会话执行一次） |
| Tests | `pnpm test` / `pnpm test:pg`（一次性 PG16 容器对拍）；`pnpm rehearsal:migrate` / `rehearsal:backup` / `rehearsal:cutover`（PG16 一次性演练）；真机飞书 runbook `repos/qm-next/docs/e2e-feishu.md` |
...
| API 契约冻结 | repos/qm-next/docs/parity-api-contract.md | 479 | 27 路由组 ~242 条 |
| PG 形状对照 | repos/qm-next/docs/parity-pg-shapes.md | - | 22 文件 ~120 用例四桶 |
| 对齐偏差全集 | repos/qm-next/docs/parity-deviations.md | - | 30+ 条跨 P1/P3/P4/P5 |
| 迁移 runbook | repos/qm-next/docs/migration.md | - | schema diff / 六类分类 / 演练 |
| 运维 runbook | repos/qm-next/docs/operations.md | - | 启动/关闭/回滚/扩缩容 |
| 切换机制 | repos/qm-next/scripts/cutover-worker-entry.ts + packages/api/src/service.ts | - | instance-registry, drain, reaper |

#### Progress

- [x] (2026-09-13) Phase P1 真引擎回路：契约冻结 →【A credentials+model ‖ B pi-harness+sandbox】→ 汇合真任务对拍 ~5d（1.0 串行门已过：`@qm/types` 四契约 + tape/LLM 记录组 + OrchestratorDeps.modelGateway，全门禁绿）（2026-09-13 完成：tag `p1`）
...
- [x] Phase P3 API 面与控制台：routes 契约 →【A api ‖ B admin/auth/portal】→ web-ui 后端化 ~5d（2026-09-14 完成：10.0 契约冻结（27 组 ~242 条全登记）→ 11.0 api 车道 tranche 1-7（~196/242 条落地）→ 12.0 admin/auth/portal 两包 + SPA + SSO → 13.0 web-ui 后端化；门禁全绿）
- [x] Phase P4 长尾 + 回填：IM 域 ‖ memory/skills/reach 完整化 ‖ 长尾子系统 → OUT 项对账 ~5d（2026-09-15 完成：14.0 IM 域四通知流+ambient 真模型+reaction-as-ack+agent-request → 15.0 memory/skills/reach → 16.0 长尾 11 tranches → 17.0 OUT 对账 + tag `p4`；test:pg 751/747+4 skip）
- [x] Phase P5 web 深化 + 迁移 + 切换：18.0 web 端深化 + 19.0 数据迁移 + 20.0 生产化 + 21.0 切换演练 + tag `v1.0.0` ~4d（2026-09-15 重整：IM 多渠道 suspended；slack 实现存 `d7d2db3` 可复活；v1 只做飞书+web；18.0 流式 SSE/deep-link/portal SSO 真路径/dark mode 全活；19.0 schema diff + migrate-qm.ts + 演练 44/44 PASS + runbook；20.0 PG-twin 接线收口 + readyz/monitoring + operations.md + 备份还原演练 PASS 100/100；21.0 instance-registry + drain + reaper + 三阶段 rehearsal:cutover PASS 11 checks；tag `v1.0.0` @ `af813d3`，test:pg 755✔/0✖/4 skip）
...
- 2026-09-14 收尾排查（2.4/3.4）：PG 对拍形状核对落 `docs/parity-pg-shapes.md`（22 文件 ~120 用例 → contract re-run/explicit/by-design/planned 四桶），补缺 3 处 PG 用例。审计逼出 run-pg.sh 真竞态：pg_isready 对 initdb 一次性服务器也报 ready，首文件（admin 排最前）撞 first-boot 重启窗口即静默 skip（exit 0 吞掉 skip，连续多轮未见）——改为 psql SELECT 1 间隔 1s 两次成功才算 settled。真模型跳过式按 #36 落四引擎 oneShot 连通性探针（env key 门），有 key 环境即可真跑
- 2026-09-15 P5 重整范围：slack/钉钉/企微 suspended（v1 只做飞书+web）；新增 web 端深化 + 监控合规生产化 + 切换演练收口。原 P5 17.0/18.0/19.0 编号改为 18.0/19.0/20.0，21.0 为切换
- 2026-09-15 19.0 迁移车道（单日收口）：qm-next 目标 schema 一律归 store 构造器所有（迁移器零 DDL、零漂移，preflight 只验表）；六类分类法（copy/transform/blob-copy/blob→cols/export-seed/not-carried/blocked-twin）让 24 张 blocked-twin 表显式成为 20.1 接线清单而非静默缺口；approvals 确认形状不可翻译（qm 存 TurnRequest 重放 vs qm-next run resume）→ 改为 drain-before-cutover 强制校验；blob-copy 沿用 qm 表名使 20.1 接线即直拷；演练驱动故意不建构造器专属 schema，把 twin-gap note 路径变成可断言行为（44/44 PASS 含 9 项语义抽查与回滚清空）
- 2026-09-15 20.0 生产化车道（单日收口）：组合根 durable sweep 把 24 张 blocked-twin 接线清单收口——sessions/runs/directory/approvals/crons/投递队列/keychain 族/model/credentials/mcp/connectors/webhooks/channel_policy/files/tasks/acl/run 观测/replay 全部 `databaseUrl` 选 PG；三处诚实改分类而非硬凑 copy——deliveries 改 drain-check（qm 行形状不可翻译为操作队列且切换前必须清空）、webhooks 数据 export-seed（记录形状不同）、tasks/task_events 移到 sessions transform 后（FK 依赖）；`createPgPool` 构造即应用 DDL 使"启动一次=全量 schema 落地"成立（pgWarmups 兜 DurableMap 惰性建表）；admin sink 常开化（观测数据≠控制台功能）；runbook/备份演练/健康探针为 21.0 切换提供运维底座
- 2026-09-15 21.0 切换车道（单日收口）：发现并修复真空缺——组合根从未跑 reaper（runbook §3"租约过期自动回收"此前无人兑现），21.3 接线 `reapIntervalMs`（默认 10s）+ 跨实例 CAS；instance-registry 接线 = PG `instance_heartbeats` + drain 控制器 + claim gate；blue-green 同 sha 双实例共存，按 build_sha 切代；灰度双跑演练 PASS 11 checks（同 sha 20/20、双 build_sha 切代 10/10、SIGKILL 接管 71 runs exactly-once）；tag `v1.0.0` @ `af813d3` 后 qm 进入只读归档，长期双跑机制留 runbook fallback 形态
...
- qm `src/slack/` 是隔离包，但 Slack 形状渗透 10+ 处 — 类型契约而非依赖切割才是关键
- 子代理配额耗尽反复出现（2.0/14.0/15.0/16.0），退路是主会话本地串行；带 brief 的 worker brief 模板仍写就（代价是 AI 占用时段）
- `pg_isready` 对 initdb 一次性服务器首启动报 ready 撞 first-boot 重启窗口静默 skip（exit 0 吞掉 skip）— `psql SELECT 1` 间隔 1s 二次成功才算 settled，否则 PG 套件首次通过是假阳
- 真模型测试靠 env key 门（pi/claude=ANTHROPIC_API_KEY、codex/opencode=OPENAI_API_KEY、pi 可 `QM_TEST_PI_MODEL_ID` 指定）—— 有 key 即真跑、无 key 干净 skip
- 写测试要断言内容而非断言 200 — `{cron:null}` 也走 200 是盲区，pg dual impl 暴露
- qm 的 IM 形状到 provider 中性化是"非显然大工程" — type 别名迁移比拆 Slack 包贵，3.0 才彻底收口

#### Outcomes & Retrospective

**交付：**

- `v1.0.0` @ `af813d3`，tag 链 `p1`/`p2`/`p3`/`p4` + `v1.0.0`
- `test:pg` 终局 **755✔/0✖/4 skip（真模型 key 门）** exit 0；`pnpm test` **700✔668/0✖**；rehearsal 三套件连跑二次（migrate/backup/cutover）全 PASS
- 5 阶段 21 个里程碑全收口，PRD/Tasks 文档归位（TOON status=completed；Open Questions 5 项均拍板）
- 30/30 qm API 路由形状兼容（`docs/parity-api-contract.md`），admin/portal/auth/web-ui 全活，lighthouse a11y 98
- 迁移演练 44/44 PASS + 回滚清空 + 备份还原 100/100；PG-twin 接线 24 张 blocked-twin 全收口
- 切换机制：instance-registry + drain + reaper（修复此前无人兑现的"租约过期自动回收"）+ blue-green 同 sha 双实例

**范围拍板（suspended，保留 git 历史可复活）：**

- im-slack（实现存 `d7d2db3`，按 `ImProvider` 契约复用飞书通道测试矩阵重启）
- im-dingtalk（Stream）、im-wecom（回调）
- 双渠道真机验收（原 v0.1.0 20.0）
- pg-boss job-queue、monitor-poller、environments/projects/webhooks 独立 store、deployment layers（aws/docker/fly/porter）— 真凭证依赖

**经验沉淀（流程 / 架构 / 纪律）：**

- **流程**：契约先行串行门 → 多车道并行（包目录隔离）→ 汇合对拍，是平移型工作的可复用模板；写测试断言内容而非 200；环境探针（PG 真启动判定）必须有二次确认；review-bot 命中的"Slack-style"等平台词是平台中性化持续纪律
- **架构**：qm `src/slack/` 隔离 + Slack 形状渗透 = 重写关键是类型契约不是依赖切割；qm `plugins/chassis` 源码级共享件 + cordis 插件理念同构让迁移心智成本低；运行时拓扑单进程下 portal 中继半边无目标，等价转换需保留信任形状
- **纪律**：偏差记 `docs/parity-deviations.md` 不静默偏离；六类分类（copy/transform/blob-copy/blob→cols/export-seed/not-carried/blocked-twin）让缺口显式成为接线清单；durable-by-default 是铁律，PG 强制生产路径
