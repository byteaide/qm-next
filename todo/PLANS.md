...
- [ ] Phase P3 API 面与控制台：routes 契约 →【A api ‖ B admin/auth/portal】→ web-ui 后端化 ~5d（2026-09-14 完成：10.0 契约冻结（27 组 ~242 条全登记）→ 11.0 api 车道 tranche 1-7（~196/242 条落地）→ 12.0 admin/auth/portal 两包 + SPA + SSO → 13.0 web-ui 后端化；门禁全绿）
- [ ] Phase P4 长尾 + 回填：IM 域 ‖ memory/skills/reach 完整化 ‖ 长尾子系统 → OUT 项对账 ~5d（2026-09-15 完成：14.0 IM 域四通知流+ambient 真模型+reaction-as-ack+agent-request → 15.0 memory/skills/reach → 16.0 长尾 11 tranches → 17.0 OUT 对账 + tag `p4`；test:pg 751/747+4 skip）
- [ ] Phase P5 多渠道 + 迁移 + 切换：slack 复活 ‖ dingtalk/wecom → 迁移器 → 双跑演练 + tag v1.0.0 ~4d（2026-09-15 重整：IM 多渠道 suspended；18.0 web 端深化完成（SSE/deep-link/portal SSO 真路径/dark mode）；19.0 数据迁移完成（schema diff + migrate-qm.ts + 演练 44/44 PASS + runbook）；20.0 生产化完成（PG-twin 接线收口 + readyz/monitoring + operations.md + 备份还原演练 PASS，rehearsal:migrate 复跑 PASS 全绿）；剩 21.0 切换 tag v1.0.0）
